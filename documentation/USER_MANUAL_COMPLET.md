...
| `/api/carte/generer_enrichissement_zone` / `appliquer_enrichissement_zone` | POST | Enrichissement LLM zones manquantes — reste dans `app.py`, pas encore porté |
...
**Frontend** (`app.js`) : badge de type "Doublon pays-entier" (`CHANTIERS_TYPE_LABEL`). `_chantiersFormatProposition()` détecte la forme `{slug_a_conserver, zones_a_retirer}` (distincte du schéma "zone complète" des deux autres types) et affiche `conservé sur : ...` / `retiré de : ...` avec le `contexte_narratif` en retrait (`↳ « ... »`) — ajouté spécifiquement pour permettre de juger un conflit à l'écran sans ouvrir `geographie/{scenario}.md`, après l'expérience du 14 sept sur les rattachements narratifs volontaires (voir §4bis). **Point de vigilance non corrigé** : ce texte narratif est inséré sans échappement HTML, cohérent avec le comportement préexistant des deux autres types (pas une régression introduite), mais un texte narratif contenant `<` ou `&` casserait l'affichage — jamais rencontré sur le vault réel à ce jour.
...
**Non testé** : application en LOT (bouton "Appliquer" global scenario/all) pour ce type — seul le chemin "chantier isolé" (`id`) a été validé en navigateur. Branche "aucun match" du bug #5 (entité hors `pays_liste`) non re-testée en conditions réelles. Aucun chantier `doublon_pays_entier` réel n'existe actuellement dans le vault — les 6 scénarios ont été nettoyés en CLI direct avant que `--write-chantiers` n'existe.
...
**Code mort restant, pas encore nettoyé** : `_ouvrirSplitPanel`, `_ouvrirPersoPanel` (`app.js`) et leurs fonctions associées ne sont plus jamais appelées depuis la fusion dans le panneau unique — laissées en place par précaution, à retirer dans une prochaine passe de nettoyage.
