...
**Providers LLM** : toutes les tâches passent par `llm_client.py`, qui route vers Claude (Anthropic) ou Mistral selon `LLM_PROVIDER` (env var) ou le réglage GUI. Défaut confirmé le 4 juillet : `mistral` / `mistral-small`, malgré une fiabilité moindre en raisonnement géographique (confabule parfois des erreurs confiantes) — `claude-sonnet-4-6` reste disponible en sélection ponctuelle dans le GUI pour la carte/coverage si besoin.
...
Génère `journaux.yaml` (éditions locales par zone) depuis les bibles géographiques.
```bash
...
| `/api/yaml` | GET/POST | Lecture/écriture d'un fichier YAML du vault (éditeur GUI, `.bak` auto) |
| `/api/yaml/form` | POST | Écriture structurée via formulaire (ex. queue.yaml) |
| `/api/yaml/append` | POST | Ajoute une entrée à un YAML existant |
...
| `/api/dashboard` | GET | Statistiques du vault (compteurs fiches, cohérence) |
| `/api/review` | GET | Panneau de review des fiches en attente |
...
### Onglet Carte — voir `USER_MANUAL_carte_et_couverture_4juillet.md` pour le workflow détaillé pas à pas (clic pays → proposition/sélection → évaluation d'impact **obligatoire** → confirmation). Cas particulier Royaume-Uni : 4 entrées `pays_liste` (Royaume-Uni/Angleterre/Écosse/Pays de Galles) pour un seul polygone GBR — sélecteur intermédiaire au clic.

### Bug récurrent connu
Confusion `pipeline_dir` (= `generator/`) vs `vault_root` (racine du vault) dans `app.py` — `geographie/` vit à `vault_root`, pas dans `pipeline_dir`. Vérifier ce point en priorité en cas de nouveau bug carte/coverage.
