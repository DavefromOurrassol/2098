...
longueur/qualité des articles générés) et le 11 août 2026 (clarté des
descriptifs GUI, validation navigateur — voir Partie 4 pour les points
...
**Ouvert en continu depuis fin juillet**, périmètre qui s'est réduit
significativement le 11 août : David a commencé à cliquer réellement
dans le navigateur, validant **10 entrées d'un coup** —
`fix_alliances_oppositions`, `trace_injection`, `fix_annee_debut_
placeholder`, `generate`, les 2 entrées de veille
(`export_prompt_veille`/`import_veille_etat_monde`), et les 4 audits
(`audit_dates_instances`, `audit_type_relation_dominante`, `audit_
etat_temporel_fin`, `audit_longueur_articles`) — désormais toutes
`gui_verified: true`. Au passage, 3 descriptifs jugés trop techniques
ont été reformulés (`fix_annee_debut_placeholder`, `trace_injection`,
`fix_alliances_oppositions`), et un correctif de lisibilité appliqué
directement à la sortie de `trace_injection.py`. Détail complet :
`USER_MANUAL_COMPLET.md` §7 (addendum du 11 août) et §2.

**Reste à tester** :
- `enrich_minimal` (7 août)
- `generate_instances` (8 août)
- Options `--ancrage-temporel` sur `create_entities`/`generate_instances`
  (8 août)
- Menu `État`/`Clandestin` refondu dans `create_entities` (mode custom,
  chantier `trajectoire`, 9 août)

**Suggestion, toujours valable** : le reste est concentré sur 2 entrées
(`create_entities`, `generate_instances`/`enrich_minimal`) — une seule
session de clic ciblée sur celles-ci pourrait clore ce point pour de bon.
