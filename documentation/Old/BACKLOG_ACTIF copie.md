...
## 🟢 2. Mois de parution (éditions datées) — codé, à tester en conditions réelles
**Scopé le 30 août, conçu et codé le 2 septembre.** Objectif double :
progression réelle du monde dans l'année (pas juste une sélection de
sujets sur un état figé) + continuité narrative entre éditions
successives. Terminologie "mois de parution" retenue (préférée à
"édition", jugée ambiguë). **Design et code terminés — reste
uniquement à tester en conditions réelles** (au-delà du dry-run déjà
réussi : 4/4 prompts assemblés sans erreur, dates correctement bornées
au mois).

**Mécanisme central** — nouveau module `edition_utils.py` : conversion
année+mois ↔ date fractionnaire (`2098.08`), tirage du jour de
rédaction dans le mois, registre `state/editions.json` (mois de
parution GLOBAL, partagé par les 6 scénarios). `build_snapshot()`
gagne un paramètre optionnel `date_edition` (rétrocompatible) ; les 3
formules `duree_effet` remplacent `2098` en dur par cette date de
référence.

**Nouveaux outils** :
- `definir_edition.py` — fixe/avance le mois de parution actif
  **indépendamment** de tout lancement de série (aucun article généré).
  Écran GUI dédié "Mois de parution du journal". `gui_verified: false`.
- `audit_sujets_edition.py` — liste les articles d'un mois de parution
  donné avec leurs métadonnées P20 existantes, en regard des
  `custom_events` déjà injectés ce mois-ci. CLI uniquement, pas encore
  au GUI.

**`generate_series.py`/`generate_manual.py`** : `annee_edition`/
`mois_edition` optionnels dans `config_series.yaml` — repli sur le mois
de parution actif si absents. **`generate.py`** : lit le mois de
parution actif en lecture seule (ne le crée/l'avance jamais), bandeau
GUI ajouté pour le confirmer avant génération.

**Production des `custom_events`** : option manuelle retenue
(`inject_custom_events.py`, nouveau champ `edition_active: true` sur
une idée de `queue.yaml`) plutôt qu'extraction automatique — jugée trop
risquée sans protocole de validation, notée comme chantier séparé
possible si le besoin se confirme. Rétrocompatibilité totale avec les
`custom_events` existants (année entière simple), aucune migration
nécessaire.

**Bug trouvé et corrigé en testant** : `definir_edition_active()`
réattribuait le même numéro d'édition en avançant plusieurs fois de
suite sans jamais générer d'article (rien n'était mémorisé entre deux
appels) — corrigé avec un registre séparé `editions_connues`,
indépendant de l'historique des articles réellement produits.

**Limites connues, non corrigées (comportement observé, pas un bug)** :
sélection des `custom_events` plafonnée par pertinence thématique (pas
de garantie de reprise dans tous les articles futurs) ; cohérence des
formulations temporelles relatives non forcée par une consigne
explicite (dépend du LLM) — **à observer en conditions réelles avant
tout correctif préventif**, cohérent avec l'approche déjà suivie sur
P17/Bug#27.

**Documentation** : section §2quinquies ajoutée à
`USER_MANUAL_COMPLET.md` (détail complet du mécanisme, des décisions et
des limites connues).

**Reste à faire** :
- Tester une vraie génération (non dry-run) et confirmer que
  `state/editions.json` s'enregistre correctement.
- Tester `inject_custom_events.py --edition-active` en conditions
  réelles (nécessite un mois de parution déjà actif).
- Cliquer dans le navigateur réel sur l'écran "Mois de parution du
  ...
  passer à `true` une fois fait).
- Observer si le risque de formulation temporelle incohérente se
  matérialise réellement avant d'envisager un correctif.
