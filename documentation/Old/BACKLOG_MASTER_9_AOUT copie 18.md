...
## ⚪ 10. Doublons de nom complet entre journalistes pro_pouvoir/opposition d'une même zone

**Trouvé le 26 août**, en marge du nouveau champ "Forcer un
intervenant précis" (point 9) — David a remarqué que la liste
d'intervenants semblait très proche entre pro_pouvoir et opposition
sur une même zone. Scan complet de `journaux.yaml` :

- **53 doublons de nom complet exact** sur **145 zones** existant à la
  fois en `pro_pouvoir` et `opposition` (~28% des zones concernées),
  répartis sur les 6 scénarios (`breakdown` 11 zones, `fortress_world`
  4, `new_sustainability` 3, `eco_communalism` 13, `policy_reform` 4,
  `reference` 6).
- Distinct d'un phénomène voisin, probablement voulu : des **noms de
  famille partagés** (pas le nom complet) entre les deux lignes d'une
  même zone, cohérent narrativement (même population locale, même
  culture, postures politiques opposées) -- pas comptabilisé dans les
  53.
- Cause probable : génération indépendante des deux lignes éditoriales
  à l'origine de `journaux.yaml`, puisant dans le même bassin
  culturel/linguistique pour une même zone -- collision statistique,
  pas un mécanisme actif qu'on alimenterait aujourd'hui.

**Décision de David : traiter demain**, pas aujourd'hui. Trois options
déjà esquissées, aucune tranchée :
1. Laisser tel quel (cosmétique, aucun mécanisme cassé).
2. Correction manuelle (David renomme un des deux doublons de chaque
   paire directement dans `journaux.yaml`).
3. Outil semi-automatisé : un renommage par LLM pour chacun des 53 cas
   (cohérent avec le contexte culturel/linguistique de la zone) —
   **53 appels LLM**, un vrai coût à mettre en balance avec l'enjeu
   cosmétique.

**Aucun fichier livré** — diagnostic seul, pas de code écrit pour ce
point.
