...
*Consolidé le 15 juillet 2026 — vue d'ensemble projet + état des sessions du 4, du 11, du 13, du 14 et du 15 juillet*
...
### 14 juillet 2026 — P22 signal 1 construit et clos (0 incohérence sur 6 scénarios), P7 étape 4 (split de zone), 2 nouveaux garde-fous géographiques, P24 étape A livrée (voir §3quater ci-dessous)
Session longue centrée sur le chantier zonage/géographie, enchaînant sur les 3 incohérences laissées en suspens le 13 juillet. Construction de **`check_origine_reelle_coherence.py`** (P22 signal 1) : parti de 13 incohérences réelles sur les 6 scénarios, ramené à 0 après plusieurs cycles diagnostic → correction, en creusant à chaque fois pourquoi un cas semblait incohérent plutôt que de corriger aveuglément — ce qui a révélé 2 bugs de données plus larges que prévu (`type_entite` manquant sur 27+ entrées, convention Groenland incohérente entre scénarios), corrigés par deux nouveaux scripts dédiés (**`check_type_entite_coherence.py`**, **`check_conventions_territoires.py`**) plutôt que contournés localement. Un vrai chaînon manquant identifié en creusant un cas (Groenland fondu avec le Danemark dans une même zone) a mené à la construction de **P7 étape 4 (split de zone)**, backend + frontend, avec détection automatique des sous-zones à faire suivre. Livraison aussi de l'**étape A de P24** (`patrons_spatiaux.py`), d'une **recherche de zone tous niveaux** dans la Carte, et d'un **orchestrateur** (`scan_geographie_complet.py`) regroupant les 4 scripts de diagnostic géographie désormais disponibles.

### 15 juillet 2026 — P27 clos (premier usage réel du split), bug zones_pays.json sur split_zone, P24 étape B livrée (voir §3quinquies ci-dessous)
Session partie du traitement des 11 cas P27 avec l'outil split (jamais utilisé en vrai jusqu'ici). Le tout premier cas réel (Écosse/`breakdown`) a révélé que `_apply_split_zone()` n'écrivait jamais dans `zones_pays.json` — même angle mort que le rename, jamais répliqué pour split. Corrigé (`_split_zone_in_zones_pays()`). Découverte au passage, pas un bug : le fond de carte Leaflet (`world.geo.json`) ne peut structurellement pas afficher un territoire infra-national séparément de son pays souverain — limite acceptée, pas corrigée. Traitement des 11 cas terminé en combinant split (Écosse/`breakdown`), réaffectation directe (Groenland/`breakdown`), et acceptation explicite des 8 autres cas comme exceptions narratives ou déjà corrects (détail dans `BACKLOG_CONSOLIDE.md`, P27). Session étendue à **P24 étape B** : garde-fou de patron spatial intégré directement dans `complete_geographie_coverage.py` (pas de script diagnostic séparé, choix de David), en avertissement uniquement — testé en conditions réelles sur un cas neutre (Guatemala/`reference`). Bonus trouvé en creusant le même fichier : `zones_pays.json` n'était écrit qu'une fois en fin de `main()` plutôt qu'après chaque scénario — même famille de risque de désynchronisation, corrigé.
...
### 27. Incohérence de plausibilité logistique — mineure, en observation, close le 15 juillet
...
**Décision prise le 11 juillet** : ne pas modifier `build_system_prompt()` dans l'immédiat — observer si ce type d'incohérence se reproduit sur d'autres tests avant d'ajouter une consigne dédiée à la cohérence géographique des déplacements inter-zones.
**Clos le 15 juillet** : recherche sur tous les articles du vault pour des formulations de transport local similaires — un seul autre résultat, faux positif (emploi métaphorique de "pirogue" dans un article du 5 juillet, sans rapport). Aucune récidive réelle confirmée. David a choisi de fermer sans ajouter de consigne, jugeant un cas isolé insuffisant pour justifier le coût (tokens + risque de contradiction avec une règle existante) d'un ajout à `build_system_prompt()`. Voir P19 au backlog.
...
### Approche top-down pour un zoning cohérent avec les scénarios — recherche et synthèse (P24)

Deuxième partie de la session, déclenchée par une question de fond de David : le pipeline géographique actuel est **bottom-up** (les zones émergent du narratif déjà écrit) sur ses deux premières passes, mais la 3ème passe (`complete_geographie_coverage.py`, couverture des pays réels) est en fait **top-down mais naïve** — elle juge un rattachement sur la seule ressemblance textuelle, sans jamais revalider contre la logique systémique du scénario. C'est la cause racine des 4 anomalies trouvées plus haut.

**Découverte clé** : le vault contient déjà, dans `variables/{variable}.md → states.{scenario}.state_logic`, le patron de structuration spatiale exact de chaque scénario (particulièrement `organisation_territoires`, mais aussi `geopolitique_conflits` et `frontieres_du_systeme`) — jamais exploité pour la génération de zones jusqu'ici. Exemple : `organisation_territoires.states.fortress_world.state_logic` = *"Archipel territorial fragmenté en zones sécurisées fortement contrôlées, avec fort contraste entre espaces protégés et zones abandonnées ou instables."*

**Recherche documentaire menée** (7 sources, lues et croisées avec le vault) :
- Raskin et al., *Great Transition* (Global Scenario Group, 2002) — rapport original à l'origine du cadre à 6 familles de scénarios ; confirme le patron `fortress_world` (une "Alliance for Global Salvation" impose l'ordre, enclaves protégées **dans les pays riches ET pauvres**, pas juste un clivage Nord/Sud)
- National Intelligence Council, *Global Trends 2040* (2021, gouvernement US, domaine public) — 5 scénarios 2040 qui recoupent presque terme à terme les 6 d'Ourrassol (`Separate Silos`≈`fortress_world`/`breakdown`, `Tragedy and Mobilization`≈`new_sustainability`, `A World Adrift`≈`reference`, `Competitive Coexistence`≈`policy_reform`)
- Meadows et al., *The Limits to Growth* (Club de Rome, 1972, accès libre Dartmouth) — grounding quantitatif des boucles de rétroaction population/capital/pollution/ressources
- Narberhaus et al., *Réussir la grande transition* (SmartCSOs, 2011) — cadre macro/méso/micro (paysage/régimes/niches), transposable à la tension régime dominant/niches dissidentes d'une zone
- d'Iribarne, *Le tiers-monde qui réussit* (Futuribles, 2004) — une institution "universelle" se réinterprète toujours localement, jamais identique d'un endroit à l'autre (leçon pour éviter des zones `new_sustainability` "copiées-collées")
- Godet, *Creating Futures* (2006) — méthode d'analyse morphologique (MICMAC/MACTOR), transposable au garde-fou (combinaisons cohérentes de variables plutôt que jugement isolé)
- Gaudin, thèse de doctorat (2008) + extraits de *2100, récit du prochain siècle* — texture atmosphérique ponctuelle, moins directement actionnable (récit linéaire unique, pas de scénarios branchants)

**Livrable de cette recherche** : `APPROCHE_ZONING_GEOGRAPHIE_SCENARIOS.md`, document de synthèse complet (grille de lecture des 6 scénarios croisant vault + sources externes, mécanique proposée pour le garde-fou étendu, design du futur générateur top-down, séquencement recommandé A→B→C). Pas de code produit cette session sur ce chantier — c'est un scoping approfondi, voir P24 au backlog.

---

## 3quater. Session du 14 juillet 2026 — détail

**Déclencheur initial** : reprendre le chantier zonage laissé en suspens le 13 juillet — 3 incohérences géographiques restantes, garde-fou P22 scopé mais pas construit, étape A de P24 pas formalisée.

### P24 étape A — `patrons_spatiaux.py` + `extract_state_logic.py`
**`extract_state_logic.py`** : parseur générique de n'importe quelle fiche `variables/{variable}.md → states.{scenario}.state_logic`. Piège trouvé en testant sur la vraie fiche `organisation_territoires.md` : les blocs `coupling_intensity` utilisent la syntaxe wikilink Obsidian comme clé YAML (`[[geopolitique_conflits]]: 95`), que `yaml.safe_load` interprète comme une liste imbriquée — clé non hashable, `found unhashable key`. Sanitisé par regex avant parsing plutôt que de modifier le vault.

**`patrons_spatiaux.py`** : formalise le patron spatial des 6 scénarios. Version d'abord figée en dur, puis rendue dynamique sur demande de David : `state_logic`/`state_logic_complementaire` sont chargés depuis le vault à chaque import (jamais recopiés à la main), testé en modifiant le vault de test et en confirmant la propagation automatique sans toucher au code. L'analyse (`patron_a_respecter`/`a_eviter`) reste écrite à la main, avec garde-fou explicite si un scénario de l'analyse disparaît du vault.

### P22 signal 1 — `check_origine_reelle_coherence.py`, de 13 à 0 incohérences

Construit sur le modèle de `check_zones_coherence.py` (avertissement, jamais de blocage — confirmé par le taux de faux positifs de 5/9 d'une première heuristique le 13 juillet). Résolution ville/région→pays en cascade : extraction directe → alias adjectival → table statique `VILLE_PAYS` → `--resolve-llm` (batch, cache jamais repayé).

**Trois bugs de résolution corrigés en testant sur le vrai vault**, chacun trouvé en creusant pourquoi un cas semblait incohérent plutôt qu'en forçant une correction :
1. Qualificatifs entre parenthèses côté ancêtre (`"Russie (Sibérie orientale)"`) non réduits à leur pays de base — tokenisation symétrique des deux côtés de la comparaison.
2. Zones racine (N1) vérifiées à tort — aucune chaîne de parenté à valider pour la racine elle-même, exclues.
3. Variantes de nommage (`"États-Unis"`/`"États-Unis d'Amérique"`) — réutilisation d'`ALIASES` depuis `check_zones_coherence.py` plutôt que duplication.

**Deux extensions ajoutées en cours de session, sur demande** :
- **Candidats** : pour chaque incohérence, recherche automatique d'une zone N1 du même scénario qui revendique déjà le pays en question. Si aucune, écriture optionnelle dans `zones_manquantes.yaml` (`--write-zones-manquantes`, même schéma que `complete_geographie_coverage.py`).
- **Racine N1** : la chaîne `parent` remontée jusqu'à une zone sans parent — nécessaire après confusion réelle de David cherchant `corridor_iberique_energetique` (niveau 2) dans la liste de la Carte, qui ne montre que les N1. La vraie racine était `nouveau_califat_barcelone`.

**Deux découvertes plus larges que le signal lui-même, en creusant les faux positifs restants** :
- **27+ entrées `origine_reelle` sans `type_entite` du tout** (ex. "Burkina Faso" listé nu) — masquait `ouagadougou_nouvelle_ctsa` et `seoul_accords` comme fausses incohérences. `_compte_comme_pays()` généralisée : seul `type_entite: ville` reste exclu, tout le reste (absent, `region_administrative`, `autre`) se fie à une correspondance exacte avec `zones_pays.json`. Découvert et corrigé en 3 vagues successives (absent → `region_administrative` sur la Polynésie française → `autre` sur le Groenland) avant de se généraliser à la règle actuelle.
- **Convention Groenland incohérente entre scénarios** : traité comme entité autonome dans certains (`fortress_world`), implicitement fondu au Danemark dans d'autres (`policy_reform`, où le candidat correct s'est révélé être `europe_nord_ouest`, pas `espace_eurasiatique` malgré une correspondance littérale valide — ce dernier bloc russo-chinois n'a aucune mention narrative d'Arctique). `VILLE_PAYS["nuuk"]` résout maintenant vers `["groenland", "danemark"]`, laisse remonter tous les candidats plausibles.

**Confirmé par David en fin de session : `--all` → 0 incohérence sur les 6 scénarios**, après plusieurs allers-retours de reparents via la Carte (Genève, Bruxelles, Hanse Baltique, Camargue, Nuuk×2, Mourmansk).

Tableau récapitulatif markdown ajouté à la sortie du script (scénario / cas / zone de départ / racine N1 / candidat), généré automatiquement plutôt que reformaté à la main à chaque run.

### P26 — `check_type_entite_coherence.py`
Diagnostic + réparation (`--apply`, backup `.bak`) des entrées sans `type_entite` découvertes ci-dessus. Édition ligne-à-ligne du texte brut plutôt qu'un re-dump YAML complet, pour ne jamais reformater tout un fichier de 3000+ lignes en corrigeant 2 lignes. Bug trouvé et corrigé avant tout `--apply` réel : une valeur `entite` repliée sur deux lignes (YAML standard, cas réel `pacte_des_souverains`) était prise à tort pour une entrée incomplète par le premier scan texte — corrigé en avançant au-delà des lignes de continuation avant de conclure. Confirmé par David : plus aucune entrée sans `type_entite` sur les 6 scénarios après `--apply`.

### P27 — `check_conventions_territoires.py` + convention décidée
Diagnostic distinct de P22 signal 1 : au lieu de comparer une zone à sa chaîne de parenté, compare un même territoire ambigu (dépendance/collectivité) **entre les 6 scénarios**. Première version se trompait en flaguant la Polynésie française (distincte de la France dans les 6/6 scénarios — un choix narratif stable, pas un bug) ; corrigée pour ne remonter que les territoires dont le traitement **varie** réellement. David a ensuite tranché une convention générale plutôt qu'un traitement au cas par cas : **les territoires dépendants/autonomes sont toujours distincts de leur souverain réel** (extension du pattern Polynésie française). Le script vérifie maintenant la conformité à cette règle. État final : 11 cas à séparer (Groenland ×3, Écosse ×3, Pays de Galles ×5), aucun traité à la fin de la session — voir P27 au backlog.

### P7 étape 4 — Split de zone
Chaînon manquant identifié en discutant comment traiter les 11 cas de P27 : ni rename ni reparent ne permettent d'extraire un sous-ensemble de `origine_reelle` d'une zone — les deux déplacent toujours la zone entière. Construit sur le modèle exact des routes impact/apply existantes (`_apply_reparent_zone`, `carte_creer_zone_niveau1`) : `impact_split_zone` (aperçu) + `split_zone` (apply, backup `.bak`).

**Itéré deux fois suite aux retours de David, chaque fois vers plus d'automatisation** :
1. Version initiale : ne déplaçait jamais les sous-zones automatiquement (jugement narratif supposé nécessaire). David a fait remarquer qu'aucun cas ne justifie de séparer un pays sans faire suivre ses sous-zones spécifiques — corrigé pour détecter automatiquement (même principe que le reste du pipeline : la propre `origine_reelle` de la sous-zone référence-t-elle aussi le pays extrait ?) plutôt que de demander un second geste manuel.
2. Cette détection ratait d'abord les variantes d'écriture (`"Groenland"` vs `"Danemark (Groenland)"` vs `"Groenland (Danemark / Kalaallit Nunaat)"` — trois formulations du même territoire, trouvées sur le cas réel `arctique_nordark`/`groenland_fonds_glaces`). Corrigé en portant la même logique de tokenisation que `check_origine_reelle_coherence.py` dans `app.py` (`_tokens_entite`, dupliquée car `gui/` et `generator/` sont deux codebases séparées) — l'API accepte maintenant un pays normalisé plutôt qu'une chaîne exacte à extraire.

Testé de bout en bout sur le cas réel Groenland/`arctique_nordark` : extraction des 2 formulations présentes dans la zone source, `groenland_fonds_glaces` reparenté automatiquement, `hanse_baltique` (lié à la Pologne, sans rapport) resté en place, YAML valide après coup, aucune nouvelle incohérence introduite (vérifié avec `check_origine_reelle_coherence.py`).

**Frontend ajouté** : bouton "✂️ scinder" dans l'arbre de la Carte (tout nœud ayant plus d'un pays, racine incluse — contrairement à "déplacer" qui exclut la racine), panneau à cocher pour choisir le(s) pays à sortir, rapport d'impact avant confirmation.

**Distinction clarifiée avec David en fin de session** : le clic sur un pays sur la carte (`carte_assign`, action `"creer"`) fait déjà une partie de ce que fait split, pour un cas simple (un seul pays, une seule écriture, pas de sous-zone liée) — mécanisme préexistant, pas besoin de split pour ces cas-là. Split n'apporte une vraie valeur ajoutée que pour les cas plus complexes (plusieurs formulations du même pays, sous-zones à faire suivre).

### Recherche de zone tous niveaux (Carte)
Nouvelle route `/api/carte/rechercher_zone` (backend) + champ de recherche dans la sidebar (frontend) : la liste principale de la Carte n'affiche que les zones niveau 1 (`_scan_n1_zones_with_desc`), rendant une zone niveau 2/3 introuvable sans connaître d'avance sa racine. Retourne le chemin complet racine→zone pour chaque résultat ; au clic, ouvre directement le bon arbre et centre/surligne la zone trouvée. Motivé par la même confusion réelle que l'extension "racine N1" de `check_origine_reelle_coherence.py` (`delta_rhone_fermes_verticales`, niveau 3, sous `corridor_iberique_energetique`, lui-même sous `nouveau_califat_barcelone`).

### `scan_geographie_complet.py` — orchestrateur
Regroupe les 4 scripts de diagnostic géographie désormais disponibles (`check_zones_coherence.py`, `check_type_entite_coherence.py`, `check_origine_reelle_coherence.py`, `check_conventions_territoires.py`) en une seule commande, sans les fusionner (chaque script reste indépendant, appelé en sous-processus plutôt qu'importé — un `sys.exit()` d'un script ne doit jamais tuer les autres). Résumé consolidé à la fin, aucune écriture par défaut.

---

## 3quinquies. Session du 15 juillet 2026 — détail

**Déclencheur initial** : traiter les 11 cas P27 avec l'outil split livré la veille — premier usage réel de ce mécanisme, jamais cliqué en vrai navigateur jusque-là.

### Bug — `_apply_split_zone()` n'écrivait jamais dans `zones_pays.json`
Premier cas réel traité (Écosse/`breakdown`, zone `archipel_anglo_celtique`) : split confirmé réussi côté logs (`impact_split_zone` puis `split_zone`, tous deux `200 OK`, aucune erreur), nouvelle zone `ecosse` bien créée et lisible via `/api/carte/arbre_zone`. Pourtant la carte affichait toujours la même couleur pour l'Écosse et le reste du Royaume-Uni. Diagnostic en creusant `zones_pays.json` directement (`grep`) plutôt qu'en se fiant au rendu carte : la clé `"Écosse"` pointait toujours vers `archipel_anglo_celtique`. Lecture du code (`_apply_split_zone`, `gui/app.py`) : la fonction écrit bien dans `geographie/{scenario}.md` (`fm["zones"]` → `geo_file.write_text`) mais ne touche jamais `zones_pays.json` — même angle mort que celui déjà corrigé pour le rename (`_rename_zone_in_zones_pays`), jamais répliqué pour split au moment de sa construction la veille.

**Corrigé** : nouvelle fonction `_split_zone_in_zones_pays(zones_pays_path, scenario, slug_source, pays_normalises, cible_slug, dry_run)`, même principe que la fonction équivalente pour rename. Appelée à deux endroits : dans le rapport d'impact (dry-run, nouveau champ `pays_zones_pays_json` pour prévisualiser l'effet avant confirmation) et dans l'application réelle (écrit le fichier, nouveau champ retourné `pays_zones_pays_json_maj`).

**Rattrapage du cas Écosse déjà à moitié fait** : la zone existait déjà côté fiche géographie au moment du patch — corrigé ponctuellement en éditant `zones_pays.json` directement (`"Écosse": "archipel_anglo_celtique"` → `"Écosse": "ecosse"` dans le bloc `breakdown`) plutôt que de refaire le split en entier.

### Limite découverte, pas un bug — fond de carte sans subdivisions infranationales
Après le correctif, la carte affichait toujours la même couleur pour l'Écosse malgré des données désormais correctes. Remonté à la source du fond de carte (`gui/static/app.js`, `loadWorldGeojson()`) : `fetch('https://cdn.jsdelivr.net/gh/johan/world.geo.json/countries.geo.json')` — un jeu de contours au niveau pays souverain reconnu par l'ONU, un polygone par pays, aucune subdivision Écosse/Pays de Galles/Angleterre/Irlande du Nord. La carte ne pourra donc **jamais** afficher ces territoires séparément avec ce fond-là, quel que soit l'état des données. Confirmé être une limite structurelle du fond de carte, pas un bug corrigible côté split ou côté données. Option pour plus tard, pas urgente : fond de carte avec subdivisions infranationales (Natural Earth admin-1, ou GeoJSON UK dédié superposé au fond mondial). Décision : accepter la limite, vérifier la cohérence via `zones_pays.json`/`check_*_coherence.py` plutôt qu'à l'œil sur la carte pour ce type de territoire.

### P27 — traitement final des 11 cas
En traitant les cas suivants, plusieurs se sont révélés être des choix narratifs légitimes de David plutôt que des oublis à corriger systématiquement :
- **Écosse/`breakdown`** : split fait (cas qui a révélé le bug ci-dessus).
- **Pays de Galles/`breakdown`** : exception narrative — l'Angleterre et le Pays de Galles restent unis politiquement dans `archipel_anglo_celtique` pendant que l'Écosse s'en détache, spécifique à ce scénario.
- **Écosse/`fortress_world` et Pays de Galles/`fortress_world`** : réglés d'un coup — le Royaume-Uni entier (4 nations) reste politiquement uni dans `bloc_atlantique`, cohérent avec la logique de repli sur les frontières existantes de ce scénario.
- **Groenland/`breakdown`** : pas un cas de mélange avec le Danemark (contrairement à l'hypothèse initiale) — Danemark et Groenland étaient déjà dans des zones différentes (`europe_du_nord` / `espace_eurasiatique`). Réglé par simple réaffectation du Groenland vers `amérique réformée`, plus cohérent géopolitiquement (plaque continentale nord-américaine) que l'ancienne zone eurasiatique — sans split, puisqu'il n'y avait rien à séparer.
- **Groenland/`eco_communalism`** : vérifié déjà correct (`arc_septentrional`, pas mélangé) — aucune action.
- **Les 6 cas restants** (Écosse/`policy_reform`, Pays de Galles ×3 : `new_sustainability`/`eco_communalism`/`policy_reform`, Groenland/`reference`) : acceptés tels quels par décision explicite de David en fin de session, plutôt que de continuer scénario par scénario — clôture assumée de P27, pas un abandon silencieux.

### P24 étape B — garde-fou de patron spatial
Choix d'architecture tranché par David avant construction : intégré directement dans `complete_geographie_coverage.py` plutôt qu'un script diagnostic autonome séparé (à la différence des 4 scripts de `scan_geographie_complet.py`) — parce que la logique voulue est différente (comparer une composition de zone au patron narratif du scénario, pas une structure de données), et parce qu'un garde-fou en avertissement gagne à être vu au moment même de la décision plutôt qu'après coup.

**Implémentation** : `build_user_prompt()` injecte `patron_spatial_prompt_block(scenario)` (P24 étape A) dans le prompt envoyé au LLM. `SYSTEM_PROMPT` étendu pour demander un champ optionnel `avertissement_patron_spatial` — uniquement en cas de doute réel, jamais un motif de rejet automatique (conforme au docstring de `patrons_spatiaux.py`, écrit la veille pour cet usage précis). `process_scenario_review()` n'a nécessité aucune modification : elle recopie déjà tel quel tout champ renvoyé par le LLM dans `coverage_proposals_{scenario}.yaml`, où il sera visible pour la validation manuelle habituelle, au même titre que le champ `avertissement` déjà existant pour les slugs inconnus.

**Testé en conditions réelles** : Guatemala temporairement retiré de `zones_pays.json` (scénario `reference`), `--review` relancé. Prompt confirmé enrichi (2490 → 2935 tokens d'entrée). Le champ d'avertissement n'est pas apparu — comportement attendu, le Guatemala dans `ameriques_multipolaires` ne contredit aucun anti-pattern du scénario `reference` (le plus conservateur des 6). Cas positif (avertissement réellement déclenché) non testé, jugé non nécessaire pour valider l'intégration technique. Guatemala restauré après test.

### Bonus — `zones_pays.json` écrit une seule fois en fin de traitement `--all`
Trouvé en examinant le même fichier pour l'étape B : `zones_pays.json` n'était persisté sur disque qu'une seule fois, à la toute fin de `main()`, après la boucle sur tous les scénarios. En cas d'interruption en cours de route (`--all` sur 6 scénarios, crash au scénario 4 par exemple), les fiches `geographie/*.md` des scénarios déjà traités restaient à jour sur disque mais `zones_pays.json` ne l'était jamais pour aucun scénario, pas même les précédents déjà correctement traités — même famille de risque de désynchronisation que le bug split_zone ci-dessus, causée cette fois par un ordre d'écriture différé plutôt qu'un oubli pur. Corrigé : nouvelle fonction `_write_zones_pays()`, appelée immédiatement après chaque scénario (`process_scenario()` en mode direct, `apply_proposals()` en mode `--apply`), l'écriture finale de `main()` devenant une confirmation redondante mais inoffensive plutôt que le seul point d'écriture.
...
- **`zones_pays.json` et `geographie/{scenario}.md` sont deux sources de vérité séparées, écrites par du code différent — vérifier systématiquement que toute fonction qui modifie une affectation zone/pays écrit bien dans les deux.** Pattern rencontré trois fois indépendamment : le rename l'a fait correctement dès le départ (`_rename_zone_in_zones_pays`) ; le split ne l'a pas fait du tout pendant un jour entier, découvert seulement en traitant un vrai cas P27 (corrigé le 15 juillet, `_split_zone_in_zones_pays`) ; `complete_geographie_coverage.py` le faisait, mais avec `zones_pays.json` différé à la toute fin d'un traitement `--all`, vulnérable à une interruption en cours de route (corrigé le 15 juillet, `_write_zones_pays`). Pour toute nouvelle fonction d'écriture touchant à une affectation géographique (P24 étape C notamment) : chercher explicitement l'équivalent déjà existant pour rename/split avant d'écrire une nouvelle logique, et écrire `zones_pays.json` immédiatement après chaque changement plutôt qu'en différé.
- **Le fond de carte Leaflet (`world.geo.json`, un polygone par pays souverain ONU) ne peut pas afficher un territoire infra-national (Écosse, Pays de Galles...) séparément de son pays souverain, même quand les données du vault sont correctes.** Découvert le 15 juillet en traitant P27 — si un split de zone ne se reflète pas visuellement sur la carte, vérifier `zones_pays.json` directement plutôt que de chercher un bug dans le split lui-même ; ce n'est correctible qu'en changeant de fond de carte (option notée, pas urgente).
...
- **`origine_reelle` (zones) est maintenant croisé avec le parent** (depuis le 14 juillet, `check_origine_reelle_coherence.py`, P22 signal 1 clos, 0 incohérence sur les 6 scénarios). Mais l'entrée `origine_reelle` elle-même peut être structurellement incomplète (`type_entite` absent) ou typée de façon inattendue (`region_administrative`/`autre` sur un territoire qui est pourtant une entrée de premier rang dans `zones_pays.json`) sans que ça saute aux yeux — toujours vérifier `_compte_comme_pays()` avant de supposer qu'un signal "aucun ancêtre pays" est une vraie incohérence plutôt qu'un trou de données.
- **`gui/app.py` et `generator/` sont deux codebases séparées, sans import croisé.** Toute logique de matching/normalisation utile aux deux (tokenisation d'une entité `origine_reelle`, alias de pays) doit être dupliquée consciemment plutôt que supposée partagée — vécu le 14 juillet avec `_tokens_entite`/`_normalise_pays` dans `app.py`, copie fonctionnelle mais distincte de `_tokens`/`_normaliser` dans `check_origine_reelle_coherence.py`. Si l'un évolue, vérifier si l'autre doit suivre.
- **Un rattachement peut être syntaxiquement valide sans être narrativement cohérent** — la zone existe bien, le pays y est bien listé, mais la description de la zone ne mentionne jamais le sujet en question (cas réel : le Groenland dans `espace_eurasiatique`, un bloc russo-chinois technocratique, découvert via `check_conventions_territoires.py` le 14 juillet). `check_origine_reelle_coherence.py` seul ne peut pas détecter ce genre de cas — il faut comparer entre zones/scénarios, pas juste remonter une chaîne de parenté.
- **Convention décidée le 14 juillet** : les territoires dépendants/autonomes (Groenland, Écosse, Pays de Galles, Polynésie française, Nouvelle-Calédonie, Guyane française) sont toujours traités comme des entités distinctes de leur souverain réel, sauf exception narrative explicitement actée pour un scénario donné (voir P27, clos le 15 juillet, pour le détail des exceptions retenues sur le Royaume-Uni). Une réouverture de P27 sur un cas déjà tranché doit être un choix volontaire, pas un oubli.

---

## 5. Prochaine session — ordre recommandé

1. **Vérifier le rendu réel de la recherche de zone dans un navigateur** — le split a été testé en vrai le 15 juillet (voir §3quinquies), mais la recherche de zone tous niveaux (`rechercher_zone`), livrée le 14 juillet, n'a toujours été testée que côté logique (syntaxe JS/HTML), jamais cliquée dans un vrai navigateur.
2. **Étape C de P24** (le générateur top-down proprement dit) — le plus gros chantier restant sur le zonage, pas commencé. Étapes A et B livrées (14 et 15 juillet). **Avant de commencer : relire le point de vigilance sur la double écriture `zones_pays.json`/`geographie/{scenario}.md` (§4)** — toute fonction d'écriture que produira étape C doit suivre le même réflexe que les correctifs du 15 juillet.
3. Reprendre le reste du backlog ⚪ : P8 (enrichissement des 426 fiches `officialise_minimal`, ~$37 estimé), P11 (intégrer `check_zones_coherence.py` au GUI), P20 (frontmatter web), P21 (journaux oraux) — ces deux derniers encore au stade scoping uniquement.
4. Quand le passage en production sera décidé : basculer le tier `strict` sur `claude-sonnet-5` dans `TASK_TIER_DEFAULTS` (une ligne).

*(P17 — test de fiabilité `mistral-small` — clos le 16 juillet, voir backlog. Bug #27 clos le 15 juillet, voir backlog.)*
