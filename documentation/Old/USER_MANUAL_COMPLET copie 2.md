...
```
État au 25 juillet : 1 entrée sans `type_entite` détectée sur `policy_reform` (`ameriques_reformees` / Groenland) -- pas encore corrigée.
...
```
Approbation : `proposition_approuvee: false → true` à la main dans `chantiers_geographie.yaml`, ou via `chantiers.mettre_a_jour_chantier(scenario, cible, proposition_approuvee=True)` en ligne de commande -- pas encore de bouton GUI dédié (onglet "Chantiers", §4.5 du handoff, pas encore construit).
Testé et validé en conditions réelles le 25 juillet : diff d'application chirurgical confirmé (`europe_occidentale_reconstructee`/`reference` -- seule la phrase problématique modifiée, aucune fuite des 21 pays gonflés de la proposition dans le vault).
...
**Section maintenance** : `extract_localisation`, `review_localisation`, `enrich_geographie`, `restructure_zones` (⚠️ script pas encore codé, entrée présente mais échouera si lancée).
...
| `/api/chantiers/appliquer` | POST | Applique en lot les chantiers approuvés d'un scénario (ou tous) |
...
| `/api/chantiers/appliquer` | POST | `{scenario}` ou `{all: true}` — délègue à `generer_zones_topdown.py --apply-topdown` en sous-processus (synchrone, pas de LLM ici) |

**Limite connue, non corrigée** : `/api/chantiers/appliquer` n'a pas de granularité "un seul chantier" — comme `--apply-topdown` lui-même, elle applique tout un scénario (ou tous) d'un coup. Nécessiterait un filtre `--id` côté `generer_zones_topdown.py`, pas fait.

**Frontend** (`index.html` + `app.js`) : liste groupée par scénario, badges type/statut/approbation, aperçu compact de la proposition générée (nom/slug/type/description, pas le YAML brut complet), boutons d'action contextuels par ligne (Générer/Régénérer, Approuver/Retirer l'approbation, Ignorer, Marquer traité, Rouvrir), bouton "Appliquer" en haut dont le libellé s'adapte au filtre scénario en cours.
