...
## 5. Récapitulatif — tout traité en première moitié de session (archivé)

*Tous les points ci-dessous étaient le "résumé actionnable" du matin —
tout est fait, gardé ici comme trace, plus rien à en tirer.*
...
## 6. Nouveautés du 2 août 2026 (deuxième moitié de session) — état à date

### 6.1 — `trace_injection.py` — ✅ livré et testé en conditions réelles
Nouvel outil de traçabilité (instance/événement/signal → origine,
propagation espace/temps, réseau causal/relationnel, usage aval en
article). Intégré au GUI (menu Type → Élément en cascade, route
`/api/trace/<slug>`). Testé par David sur plusieurs slugs réels,
fonctionne. Détail complet en §2 du `HANDOFF_2_AOUT.md`.

**Non fait, à envisager si besoin** : onglet GUI dédié avec rendu visuel
(comme Chantiers) plutôt que le panneau générique de résultat de script —
nécessiterait `gui/app.js`/`index.html` en plus de ce qui a déjà été
fourni (déjà utilisés pour le mode Forcer, donc disponibles si besoin).

### 6.2 — Rotation à mémoire des instances — ✅ livré et testé
Les instances sélectionnées automatiquement (mode Semi-guidé) ne
retombent plus systématiquement sur les mêmes 6 à chaque article —
rotation à mémoire calquée sur celle déjà existante pour les signaux.
Testé fonctionnellement (score de pertinence toujours prioritaire,
rotation ne départage que les ex-aequo).

### 6.3 — Mode "Forcer un élément" — ✅ livré, testé en conditions réelles par David
Refonte majeure de `generate.py` + GUI : deux modes (Semi-guidé inchangé
/ Forcer), génération en lot (un article par scénario retenu), zone qui
remplace le choix de scénario plutôt que de le filtrer, élément forcé
toujours garanti présent. **8 bugs réels trouvés et corrigés en cours de
route** (détail complet en §3 du `HANDOFF_2_AOUT.md`), le plus sérieux
étant que 7 champs du formulaire (Thématique, Ligne éditoriale, Longueur,
Angle, Scénario, Zone, Titre suggéré) n'étaient jamais lus côté serveur
depuis le passage à un mécanisme CLI — silencieusement ignorés,
`generate.py` retombait sur d'anciennes valeurs figées dans `config.yaml`.

**⚠️ Point de vigilance pour la prochaine session** : ce bug touchait
potentiellement aussi le mode **Semi-guidé** (le scénario du menu
peut-être jamais réellement pris en compte avant le correctif) — corrigé
en même temps que le mode Forcer, mais **pas explicitement retesté en
conditions réelles en Semi-guidé** depuis. À vérifier en priorité.

### 6.4 — Passage à l'échelle événements/géographie — ✅ codé et testé en sandbox, ⚠️ pas testé en conditions réelles
Les événements custom et les zones géographiques n'étaient soumis à
aucun plafond (contrairement aux instances/signaux, déjà protégés) —
risque de croissance non maîtrisée du prompt à mesure que le vault
grandit. Corrigé avec le même principe partout : couche large peu
coûteuse (vision globale préservée) + couche détaillée filtrée par
pertinence/rotation (réutilise `portee` et `impact_sur_variables`, déjà
standardisés sur les fiches, pas une nouvelle heuristique). Élément forcé
toujours garanti présent en détail complet.

**Non testé en conditions réelles** — seulement validé fonctionnellement
avec des données synthétiques en sandbox. **Priorité n°1 de la prochaine
session.**

---

## 7. Résumé actionnable pour la prochaine session

1. **Tester en conditions réelles le plafonnement événements/géographie**
   (§6.4) — jamais vérifié hors sandbox.
2. **Revalider le mode Semi-guidé** après le fix du bug de lecture CLI
   (§6.3) — seul le mode Forcer a été retesté depuis.
3. Ajuster si besoin les plafonds par défaut (8 événements détaillés/25
   en résumé, 20 zones en résumé) — jamais discutés en détail avec David.
4. Envisager, si le temps le permet, l'onglet GUI dédié pour
   `trace_injection.py` (§6.1).
5. Rien d'autre identifié comme ouvert dans le backlog historique — le
   grand audit de ce document est épuisé.
