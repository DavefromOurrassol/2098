...
*Référence à jour au 2 septembre 2026 — couvre `generator/` (42+ scripts
...
## 2quinquies. Mois de parution — éditions datées (2 septembre 2026)
...
```
`gui_verified: false` — logique testée en CLI, pas encore cliqué dans
le navigateur réel.
...
### `audit_sujets_edition.py` 🔁 🗄️ — diagnostic pur, nouveau
Liste les articles générés pour un mois de parution donné (métadonnées
P20 déjà existantes : `chapo`, `zone_principale`, `thematique` — aucun
nouveau champ article nécessaire) et, en regard, les `custom_events`
déjà injectés pour ce même mois. Ne décide jamais à la place de David
quel sujet mérite injection — gain de temps de lecture uniquement, pas
de jugement de pertinence automatique.
```bash
...
```
Pas encore intégré au GUI (CLI uniquement).
...
- **Sélection non garantie à chaque article** : `select_relevant_events`
  plafonne à 8 événements par pertinence thématique — un `custom_event`
  injecté un mois donné n'apparaîtra dans les articles suivants que
  quand jugé pertinent, pas systématiquement partout.
...
### Fichiers touchés (session du 2 septembre)
...
**Pas encore testé en conditions réelles** au-delà d'un dry-run réussi
(4/4 prompts assemblés sans erreur, dates correctement bornées au mois) —
génération réelle, `inject_custom_events.py --edition-active` et
l'écran GUI "Mois de parution du journal" restent à valider par David.
...
| `/api/edition/active` | GET | Mois de parution actif (`state/editions.json`, lecture directe). Ajouté le 2 septembre 2026 (§2quinquies) — alimente le bandeau GUI sur "Générer un article" et "Mois de parution du journal" |
