...
## ⚪ 8. Localisation — slug de zone `istanbul` inconnu (`gelecek_meclisi_policy_reform`)
**Nouveau, 17 août.** Découvert au passage lors du cycle post-injection
automatique déclenché par la génération d'une instance sans rapport
(`institut_des_seuils_demographiques`/`new_sustainability`) —
`extract_localisation.py` retraite systématiquement toutes les fiches
`review_manuelle` en attente à chaque run, pas seulement celle qui vient
d'être créée. `[VALIDATION ÉCHOUÉE] slug zone inconnu : 'istanbul' (non
trouvé dans geographie/policy_reform.md)` — le LLM propose `istanbul`
comme zone pour le siège du Gelecek Meclisi, mais ce slug n'existe pas
dans la géographie du scénario `policy_reform` (zone inexistante, ou
existante sous un autre nom — non vérifié). `validate.py` : reste
l'unique avertissement `[LOCALISATION]` du vault, statut inchangé
(1 fiche sans localisation), pas bloquant. Pas de diagnostic approfondi
fait le 17 août (repéré en marge d'un autre chantier) — à investiguer :
vérifier si une zone équivalente existe dans `geographie/policy_reform.md`
sous un nom différent (auquel cas corriger le prompt/mapping), ou si
c'est un vrai trou de couverture géographique à combler.
