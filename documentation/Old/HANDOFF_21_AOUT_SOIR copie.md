...
## 15. Point de reprise réel pour la prochaine session

1. **Lancer `rapprocher_articles.py` en réel** sur le corpus complet
   (56 articles désormais tous éligibles) — jamais encore fait, priorité
   naturelle d'ouverture de la prochaine session.
2. **Réexaminer `gelecek_meclisi`** avec `--stats` une fois le point 1
   fait, sur un échantillon bien plus large que les 7 articles actuels.
3. **P25** — continuer à observer, envisager la piste du pattern `---`
   en fin de texte si le taux se confirme sur un futur batch.
4. Reste inchangé : points 3 (P17), 4 (Bug #27), 5 (renommage YAML), 6
   (troncatures JSON), 7 (P21/P14), 8 (GUI `promote_ville.py`) — tous
   en attente sur décision explicite de David.

Rien en suspens de façon bloquante. Séance longue mais tous les
chantiers ouverts ce soir ont été menés à leur terme.
