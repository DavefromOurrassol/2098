...
## 🟢 9bis. `articles_lies` + vocabulaire des tags — `rapprocher_articles.py` (codé, pas encore lancé sur le corpus complet)
...
**Testé intégralement sur corpus synthétique** (rapprochement trouvé/
absent, séparation stricte par scénario confirmée, ligne "Voir aussi"
combinée et idempotente sur relance, `--stats` avec entité
artificiellement omniprésente détectée à 100%) — jamais encore lancé
en réel sur l'ensemble du corpus.

**Découverte réelle en testant `--stats` sur un tout petit échantillon
(7 articles, 2 scénarios)** : `gelecek_meclisi_{scenario}` apparaît à
**100%** sur `breakdown` (3/3) et `policy_reform` (4/4). Investigation
menée : ce n'est pas un artefact aléatoire — l'entité a délibérément un
spectre de `variables_influencees`/`zone_systemique` très large
(gouvernance/société/information, portée continentale/globale),
recoupant quasiment toute thématique. `filter_instances_for_thematique()`
la favorise donc structurellement, et la rotation à mémoire
(`_select_least_used_instances()`) ne peut pas la départager si son
score reste systématiquement au-dessus du lot plutôt qu'à égalité avec
d'autres candidates. **Décision de David : observer sur un corpus plus
large avant de trancher** — 7 articles est trop peu pour juger si c'est
un vrai problème de diversité narrative ou une caractéristique voulue
(institution "tissu conjonctif" du monde). À réexaminer avec `--stats`
une fois `rapprocher_articles.py` lancé sur le corpus complet (point 12
...
réel le soir même. Pour la prochaine session, dans l'ordre suggéré :*
- **Point 9bis (`rapprocher_articles.py`)** — codé et testé, jamais
  encore lancé pour de vrai sur le corpus complet (56 articles
  désormais tous éligibles depuis la clôture du point 12). Premier
  geste naturel de la prochaine session.
- **`gelecek_meclisi` quasi-omniprésente** (point 9bis) — observation
  faite sur 7 articles seulement, à réexaminer avec `--stats` une fois
  `rapprocher_articles.py` lancé sur les 56.
- **P25 (signature journaliste)** — nouveau symptôme observé ce soir
