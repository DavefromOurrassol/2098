...
invalide — voir `HANDOFF_15_AOUT.md`). Remplace tous les documents
...
la génération). Sinon, confirmer sur plusieurs générations futures que le
3e correctif du chantier `forces_attractives`/`forces_repulsives`
(couverture des variables pilotes) tient dans la durée, un seul test
positif à ce stade. Reste sinon en Partie 1 : le point #1 (validation
retry longueur, toujours 🟡, sans urgence), P17/Bug#27/renommage
YAML/troncatures JSON (tous rouverts ou gardés pour plus tard sur
