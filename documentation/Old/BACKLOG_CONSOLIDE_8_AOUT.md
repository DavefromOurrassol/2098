...
1. Trancher 4 points précis (les 3 anciens tableaux de définitions ne
   sont plus la bonne base — l'Option B les rend partiellement caducs,
   voir `HANDOFF_8_AOUT.md` §12bis étape 7bis) :
   - Ordre exact final du nouvel axe, avec `marginal` replacé (oublié
     dans la première proposition).
   - Sort d'`actif` — supprimé, ou conservé comme valeur générique ?
   - Sort d'`historique` vs `mythifié` — fusionnés en un seul point
     terminal, ou gardés distincts (référence factuelle vs légendaire) ?
   - Validation séparée du tableau `generation` (7 valeurs, inchangé,
     non concerné par la fusion).
2. Affiner les règles de migration des 710 fiches existantes vers le
   nouvel axe une fois les 4 points ci-dessus tranchés.
