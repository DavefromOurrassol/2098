...
### Étape 7bis — Clarification post-décision : les 3 tableaux de définitions inférées (étape 6) sont partiellement caducs sous Option B
Question de David après coup : les définitions inférées de l'étape 6
restent-elles valables telles quelles maintenant que la fusion est
décidée ? Réponse : non, pas telles quelles — l'Option B dissout la
plupart des valeurs de `etat_temporel` dans le nouvel axe. Ce qui reste
réellement à valider par David a changé de nature :

**`etat_temporel`** — la plupart des valeurs se retrouvent absorbées dans
le nouvel axe, avec des points encore ouverts :
- `actif` : sort incertain — probablement supprimé en tant que valeur
  distincte (être positionné n'importe où sur le nouvel axe implique déjà
  d'être actif), mais pas tranché — a-t-on encore besoin d'un état
  "actif" générique pour une fiche qu'on ne veut pas positionner
  précisément ?
- `clandestin` : confirmé, sort du champ → `est_clandestin: true/false`.
- `transformé`/`disparu` : conservées telles quelles sur le nouvel axe.
- `historique` vs `mythifié` : pas tranché — un seul point terminal fusionné,
  ou deux nuances distinctes gardées en fin d'axe (référence factuelle vs
  référence légendaire — a du sens narrativement) ?

**`age_historique`** — devient l'ossature du nouvel axe, avec un oubli
identifié après coup : **`marginal` était absent** de l'axe proposé à
l'étape 7 (`émergent → ascendant → dominant → mature → déclinant →
résiduel → transformé → disparu → mythifié/historique`) — à replacer
(entre `émergent` et `ascendant` ? en branche parallèle pour une entité
qui reste petite sans jamais grandir ?). Les 6 autres valeurs (`émergent`,
`ascendant`, `dominant`, `mature`, `déclinant`, `résiduel`) forment déjà
le corps de l'axe tel que proposé.

**`generation`** — **non concerné par la fusion, le tableau de l'étape 6
reste valide tel quel** (7 valeurs : `pré-crise`, `transition`, `post-
effondrement`, `IA-native`, `forteresse`, `reconstruction`, `ère
cognitive`) — à valider séparément, indépendamment du reste du chantier.

**Ce qui doit être tranché en priorité à la prochaine session, à la
place de l'ancienne liste de définitions par champ** :
1. L'ordre exact final du nouvel axe unique, avec `marginal` replacé.
2. Le sort d'`actif` (supprimé ou conservé comme valeur générique).
3. Le sort d'`historique` vs `mythifié` (fusionnés ou distincts).
4. Validation du tableau `generation` (inchangé, indépendant des 3 points
   ci-dessus).
...
1. **Trancher les 4 points précis identifiés en étape 7bis** (§12bis) :
   ordre exact final du nouvel axe avec `marginal` replacé, sort d'`actif`
   (supprimé ou conservé comme valeur générique), sort d'`historique` vs
   `mythifié` (fusionnés ou distincts), validation séparée du tableau
   `generation` (inchangé, non concerné par la fusion).
