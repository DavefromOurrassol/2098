...
cohérence événements custom — voir `HANDOFF_12_AOUT.md`). Remplace
...
## 🟡 2. Dimension temporelle pour la génération automatique
...
première) :
- **Auto-suggest** (`analyze_entity_coverage()` côté entités,
  `analyze_vault_coverage()` côté événements) mesure déjà 3 dimensions
  (géographie, zones absentes, catégories/variables) dans les deux
  cas — ajouter la distribution d'`annee_debut`/`date` par bande
  temporelle, pour que l'auto-suggest propose activement des créations
  dans les bandes sous-représentées. **Portée élargie le 12 août** :
  confirmé que ce gap touche identiquement les deux pipelines
  (entités ET événements), pas seulement les entités comme supposé au
  8 août.
- **Répartition homogène + ancrage sur les crises réelles** pour la
  ...
  côté instances), et ancrer sur les crises du registre du scénario
  plutôt qu'une répartition arithmétique/aléatoire.

---

## 🟢 3. Documentation à corriger (chantier `trajectoire`, 9 août)
...
## 🟢 4. Doublon potentiel d'entité — `arctic_passage_authority` / `autorite_passage_arctique`
...
## 🟢 5. Nettoyage `test_durcissement_policy_reform`
...
## 🟢 6. Quatre reliquats jamais repris depuis la consolidation du 7 août
...
## 🟢 7. Renommage des YAML génériques par dossier
...
## 🟢 8. Fichiers parasites dans `generator/` (incident du 5 août, diagnostiqué le 11 août)
...
## 🟢 9. Troncatures JSON occasionnelles lors de la génération d'instances (Mistral)
...
depuis la Partie 1 (2 points 🟡 seulement — retry longueur à plus grande
échelle, dimension temporelle génération auto —, plus aucun 🔴 en
attente). Le chantier "Test navigateur GUI", ouvert en continu depuis fin
juillet, est définitivement clos — les 28 entrées du panneau sidebar sont
toutes `gui_verified: true`. Le correctif signature (itération 2), le
diagnostic `annee_debut`/`ancrage_reel` sur les événements, le nouveau
chantier de cohérence événements custom (ancrage sur l'état du monde réel
+ validation géographique), et le correctif du panneau Revue
(slug/scénario/détail vides sur entités/signaux + repli générique
`error:` sur les 3 pipelines) sont tous clos depuis le 12 août. Fichiers
livrés le 12 août (voir `HANDOFF_12_AOUT.md` pour le détail) :
`inject_custom_events.py`, `app.py`.*
