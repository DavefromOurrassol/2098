...
## ✅ 10. P25 — Fiabilité de la signature journaliste dans le corps de l'article — clos (23 août)
...
**Proposition non codée le 22 août, implémentée et confirmée le 23 août** :
voir section "Cause racine trouvée et corrigée" ci-dessous.

### Cause racine trouvée et corrigée le 23 août — remontée de zone N1/N2/N3

**Point soulevé par David** : la géographie du vault a une hiérarchie de
zones (niveau 1/2/3, champ `parent`), jamais prise en compte dans le
diagnostic du 22 août. Vérification : `journaux.yaml` n'a **jamais
qu'une entrée par zone NIVEAU 1** (déjà documenté le 12 août pour un
cas similaire — `--zone-slug` de `generate.py` proposait des
sous-zones sans journal, corrigé à l'époque **uniquement pour la
sélection manuelle GUI**, jamais pour la résolution automatique).

**Cause confirmée** : `_dominant_zone()` (`snapshot.py`) calcule
`zone_slug` par vote majoritaire sur la localisation des instances
filtrées — peut retourner le slug d'une **sous-zone niveau 2/3** si les
instances de l'article y sont localisées. `get_journal_profile()`
cherchait ce slug directement dans `journaux.yaml` (structure niveau 1
uniquement) sans jamais le trouver, tombant sur le chemin 2/3 (réseau
global/profil hardcodé — aucun nom de journaliste curaté disponible,
le LLM reçoit l'instruction "invente un nom toi-même") — **même quand
une zone précise ET rattachable à un N1 réel avait bel et bien été
résolue**, pas seulement dans le cas `zone_slug is None` regardé la
veille.

**Mesure empirique du résidu avant correctif** : 13/111 articles
(≈12%) avec `zone_principale` totalement vide dans le frontmatter —
plancher du problème, pas le total réel (le cas N2/N3 corrigé ici
laisse `zone_principale` rempli avec un slug de sous-zone, donc
invisible à cette mesure).

**Correctif** : nouvelle fonction `_resoudre_zone_n1(zone_slug,
scenario_slug)` (`prompt_builder.py`) — remonte la hiérarchie via le
champ `parent` (même principe que le breadcrumb `chemin` de la Carte,
`app.py`, `/api/carte/rechercher_zone` : garde-fou anti-cycle inclus)
jusqu'à la zone niveau 1 la plus proche, **avant** toute recherche dans
`journaux.yaml`. Namespace de rotation des journalistes
(`_select_journaliste_pondere()`, chantier du 22 août) également basé
sur `zone_slug_n1` plutôt que `zone_slug` brut — une sous-zone et sa
zone N1 partagent la même rédaction, les compter séparément aurait
fragmenté inutilement le garde-fou anti-oubli.

**Testé sur 6 cas synthétiques avant tout déploiement**, dont le cas
réel exact documenté le 12 août (`archives_neutres_geneve`, N3 → N1 en
2 sauts) : tous passent, y compris les deux garde-fous critiques
(cycle détecté → pas de boucle infinie, géographie absente → pas pire
qu'avant le correctif).

**Confirmé en conditions réelles sur 2 scénarios indépendants** :
`fortress_world` (le plus touché historiquement, ~25-33% d'échec les
21/22 août) → **100% de réussite** sur un batch de 20 articles, comme
`new_sustainability` la veille. Preuve solide que la cause était bien
la résolution de zone, pas une variance d'échantillon.

**Fichier livré** : `prompt_builder.py`.

**Chantier définitivement clos** — décision de David après 2 scénarios
à 100%, pas besoin de re-tester le 3e (`policy_reform`).
...
## ✅ 15. Rotation pondérée des journalistes par séniorité — clos (22 août)
**Demande de David**, née en creusant P25 : `get_journal_profile()`
(`prompt_builder.py`) choisissait toujours, de façon **déterministe**,
le·la premier·ère journaliste d'une zone dont `thematiques` correspond
à l'article — figé pour toujours sur le même nom pour une thématique
donnée d'une zone donnée, aucune variété. David voulait une rotation,
mais pondérée par une note de séniorité (probabilité de retour plus
haute pour une séniorité plus élevée), **sans qu'aucun journaliste ne
soit jamais complètement oublié**.

**Conception, dans le même esprit que le cooldown des instances du même
jour** : nouvelle fonction `_select_journaliste_pondere()` —
1. Tirage aléatoire pondéré par `seniorite` (`random.choices`, poids =
   séniorité, défaut `1` si absente — non-régression pour toute entrée
   pas encore mise à jour).
2. **Garde-fou anti-oubli**, non comparatif (donc insensible au défaut
   mathématique qui avait invalidé la pénalité de score des instances
   plus tôt dans la journée) : au-delà de
   `JOURNALISTE_MAX_ABSENCE_STREAK = 5` apparitions consécutives sans
   être choisi·e (parmi ses occasions réelles d'être choisi, pas un
   compteur absolu), un·e journaliste est sélectionné·e d'office,
   indépendamment de sa séniorité.

**État persistant réutilisé** : même fichier que la rotation des
jalons (`TRAJECTORY_STATE_FILE`), nouveau namespace
`"journalistes::{ligne}::{zone_slug}"` sous `usage_state[scenario_slug]`
— même convention de clé que l'existant, pas de nouvelle structure.
Chargé/sauvegardé de façon autonome dans `get_journal_profile()` ;
vérifié que `build_system_prompt()` est appelé **avant**
`build_trajectory_context()` dans `build_prompt()`, donc aucun risque
d'écrasement entre les deux sauvegardes séquentielles sur le même
fichier. `dry_run` propagé (nouveau paramètre sur
`get_journal_profile()`/`build_system_prompt()`) — un aperçu de prompt
ne consomme pas une "chance" de rotation, même convention que la
rotation des jalons.

**Testé sur cas synthétiques avant tout déploiement** (3 cas, 200+90
tirages) : rotation pondérée respectée en tendance (séniorité 5 → 99
sélections sur 200, séniorité 1 → 44) tout en gardant l'absence
maximale observée strictement plafonnée à 5 pour chaque candidat (le
garde-fou se déclenche systématiquement avant l'oubli) ; candidat
unique (non-régression, pas de plantage) ; aucune séniorité définie
nulle part (non-régression totale, répartition équilibrée par défaut).

**`journaux.yaml` enrichi** : `seniorite: 1` (valeur par défaut) ajouté
aux 1740 journalistes du fichier (290 zones, 3 scénarios vérifiés dans
cette session), sans écraser une valeur déjà présente si le fichier
avait été partiellement édité à la main. Intégrité vérifiée
programmatiquement (noms/thématiques strictement identiques à
l'original, seule `seniorite` ajoutée). **Point de vigilance mineur** :
le round-trip YAML a reformaté certains champs texte longs (`ton`,
`charte`, `langue_style`) d'un repli multi-lignes vers une ligne
unique large — contenu textuel inchangé, mise en forme seulement,
peut produire un diff Git plus large que prévu sur ces champs
spécifiquement.

**Non-régression vérifiée sur les 3 autres scripts consommant
`journaux.yaml`** (repo complet inspecté via `generator.zip`) :
- `check_journaux_coherence.py` : lecture seule, ne compare que les
  slugs de zones — aucun risque.
- `clean_fallback_journaux.py` : recharge tout le fichier en dict
  Python, ne retire que les zones placeholder (aucune zone réelle du
  vault ne matche ce motif), réécrit l'intégralité de la structure —
  `seniorite` traverse ce round-trip sans perte.
- `generate_journaux.py` : mode `--update` (par défaut) ne touche que
  les zones absentes ; mode `--fill-journalistes` ne remplit que les
  zones où `journalistes` est vide (aucune, désormais) — aucun des
  deux modes courants ne réécrase `seniorite`. Seul un lancement
  explicite sans `--update` (régénération complète) l'effacerait —
  pas l'usage courant, mais à garder en tête.

**Fichiers livrés** : `prompt_builder.py`, `journaux.yaml`.

**Reste à faire, pas fait aujourd'hui** : ajuster manuellement les
valeurs de `seniorite` (au-delà du défaut uniforme à 1) pour les
journalistes que David veut effectivement favoriser — remis à plus
tard, la mécanique est prête à recevoir ces valeurs dès qu'elles seront
définies. Pas testé en conditions réelles (aucun run `generate.py`
lancé après ce correctif) — à confirmer à la prochaine génération, via
la nouvelle clé `journalistes::{ligne}::{zone}` dans
`state/trajectory_usage.json`.

**Confirmé en conditions réelles le 23 août** : rotation observée sur
plusieurs batches (`fortress_world`, `new_sustainability`) — noms de
journalistes variés d'un article à l'autre pour une même thématique de
zone, plus le figement déterministe d'avant ce chantier.

**Suite majeure le 23 août (après-midi) : voir point 19 ci-dessous** —
diagnostic du 22 août confirmé beaucoup plus sévère que prévu (96-98%
des combinaisons zone×thématique à un seul journaliste éligible sur la
plupart des scénarios), nouvel outillage complet construit pour
combler ces trous (audit, propositions, injection manuelle/auto).

---

## ✅ 19. Outillage complet de couverture des journalistes — construit et partiellement testé (23 août, après-midi)

**Point de départ** : en croisant deux articles quasi identiques
(`bassin_du_congo`/`petites_annonces_services`, même journaliste,
même 6 entités citées), diagnostic confirmé — `Samira Benyahia` était
la **seule** journaliste de sa zone à couvrir cette thématique (sur 6).
Pas un bug de rotation : un trou de couverture dans les données
elles-mêmes, la rotation du 22 août n'avait simplement personne
d'autre à proposer.

### `audit_couverture_journalistes.py` (nouveau, lecture seule)

Balaie `journaux.yaml` entier et compte, pour chaque combinaison
scénario × ligne × zone × thématique, le nombre de journalistes
réellement éligibles. Signale les combinaisons à 0 (repli sur toute la
liste, rotation possible mais sans lien thématique) et à 1 (toujours
le même nom, aucune rotation possible).

**Résultat réel sur le vault complet** (mesuré en session) :
`breakdown` 96%, `eco_communalism` 96%, `fortress_world` 49% (après un
premier passage du mode auto, voir plus bas), `new_sustainability` 98%,
`policy_reform` 98%, `reference` 96% de combinaisons fragiles. Confirme
que le trou diagnostiqué sur `bassin_du_congo` n'était pas un cas
isolé — c'est structurel, hérité de la conception d'origine de
`generate_journaux.py` ("chaque thématique couverte par au moins
un·e", jamais pensé pour permettre une rotation).

**`--report`/`-r` ajouté en cours de session** (même pattern que
`validate.py`) : écrit le détail complet dans
`audit_couverture_journalistes_report.txt`, ne laisse que le résumé
par scénario dans le terminal.

**Résumé qualitatif ajouté** (retour de David : "96% ne parle pas") :
chaque pourcentage est maintenant accompagné d'une traduction en
langage clair (5 paliers, de "rotation possible partout" à "quasiment
aucune rotation possible -- presque toujours le même nom").

**GUI** : nouvelle entrée section `validation`, jamais testée dans le
navigateur.

### `propose_couverture_journalistes.py` (nouveau, lecture seule)

Prolonge l'audit : pour chaque trou détecté, propose d'ajouter la
thématique manquante à 1-2 journalistes **déjà existants** de la même
zone (priorité à ceux qui ont le moins de thématiques actuellement,
pour répartir la charge). Ne modifie jamais `journaux.yaml` — simple
proposition à valider/appliquer manuellement, même principe que
`needs_review.yaml` ailleurs dans le pipeline. CLI uniquement, pas
d'intégration GUI (décision explicite de David).

### `inject_journaliste_custom.py` (nouveau, mode manuel + mode auto)

**Mode manuel** : ajoute un journaliste précis à une zone déjà
existante, généré par LLM avec un profil cohérent avec le ton déjà
établi de l'édition (réutilise `zone_data["ton"]`/`["langue_style"]`,
même esprit que `generate_journaux.py` d'origine).

**Options ajoutées en cours de session, sur demande de David** :
- `--nom` : si fourni avec `--thematiques`, **aucun appel LLM** — patch
  YAML direct. Si fourni seul, le LLM ne génère que les thématiques.
  Absent, comportement d'origine (LLM génère tout).
- `--genre` (homme/femme/vide) : oriente le LLM sur le genre du
  personnage inventé — sans effet si `--nom` est déjà fourni.
- `--seniorite` (défaut 1, même convention que les 1740 journalistes
  du 22 août) — **bug initial trouvé par David** : les nouvelles
  fiches n'avaient pas ce champ du tout, corrigé.

**Mode auto** (nouveau) : scanne un scénario entier (`--scenario` ou
`--all` pour les 6 d'affilée, ajouté en cours de session), et pour
chaque thématique sous la cible de couverture choisit intelligemment
entre redistribuer (journaliste existant sous le plafond
`MAX_THEMATIQUES_PAR_JOURNALISTE = 6`) ou créer un nouveau profil par
LLM (si tous les existants sont déjà au plafond). Testé en synthétique
sur 4 cas (redistribution simple, tous au plafond, déjà à la cible,
cas mixte) — tous passent.

**Testé en conditions réelles** : mode manuel confirmé de bout en bout
(nom imposé + genre + précision de profil + thématiques déduites par
LLM, `seniorite` bien écrite). Mode auto lancé une fois sur
`fortress_world` : **396 redistributions + 21 créations** en un seul
run, faisant chuter la fragilité de ce scénario de ~96-98% (valeur
implicite avant, jamais mesurée directement) à **49%** mesuré après —
confirmation nette que le mécanisme fonctionne, mais qu'une seule
passe ne suffit pas à tout combler (capacité de redistribution limitée
par le plafond, 21 créations sur cette passe seulement).

**`--all` jamais testé** — ajouté en toute fin de session, reporté à la
prochaine fois (décision explicite de David, pas de budget pour un
test potentiellement coûteux en fin de séance).

### 3 bugs GUI trouvés et corrigés en cours de route

- **`mode_only` jamais respecté par la validation avant lancement**
  (`app.js`, `validateRequiredFields()`) — un champ masqué par le mode
  actif (ex. `--zone-slug` en mode auto) restait `required`, bloquant
  le lancement sur un champ invisible. Trouvé sur cette nouvelle
  entrée (jamais rencontré avant : aucune entrée existante ne
  combinait `mode_select` et un champ `required` restreint par
  `mode_only`).
- **Même trou côté collecte** (`collectArgs()`) — un champ masqué
  pouvait quand même voir sa valeur envoyée dans la commande. Corrigé
  par la même occasion (même logique de filtrage par mode actif),
  révèle au passage un second cas latent : deux champs `--ligne`
  distincts (un par mode) partageant le même flag auraient pu se
  télescoper sans ce correctif.
- **`--thematiques` : mauvaise convention de sérialisation supposée**
  — le champ `multi_select` du GUI envoie plusieurs arguments séparés
  par des espaces (`nargs="+"`, convention déjà établie dans
  `inject_custom_events.py`), pas une chaîne séparée par virgules comme
  je l'avais supposé au départ. Corrigé.

### Dette technique notée, non traitée

Trois implémentations indépendantes du même calcul de couverture
zone×thématique (`audit_couverture_journalistes.py`,
`propose_couverture_journalistes.py`, `mode_auto` d'`inject_journaliste_
custom.py`) — repéré par David, laissé tel quel par manque de temps en
fin de session plutôt que factorisé. Fonctionne, mais risque de
maintenance si la logique de calcul change un jour (à corriger aux
trois endroits).

**Fichiers livrés** : `audit_couverture_journalistes.py` (nouveau),
`propose_couverture_journalistes.py` (nouveau),
`inject_journaliste_custom.py` (nouveau), `app.js`, `scripts_config.json`.

**Reste à faire** :
- Lancer le mode auto sur les 5 autres scénarios (`breakdown`,
  `eco_communalism`, `new_sustainability`, `policy_reform`, `reference`)
  — jamais fait, seul `fortress_world` a été traité.
- Tester `--all`.
- Vérifier le rendu GUI (jamais ouvert dans un navigateur).
- Éventuellement une seconde passe sur `fortress_world` (49% encore
  loin de 0%, la capacité de redistribution était limitée sur la
  première passe).

---

## ✅ 20. Format `petites_annonces_services` ne ressemblait pas à une annonce — clos, deux mécanismes distincts, testés en conditions réelles (23 août)

**Important : deux correctifs indépendants, pas liés entre eux malgré
la même thématique déclenchante** — David a explicitement clarifié en
session que le second (longueur figée) n'a **aucun rapport** avec P21/
l'oralité, contrairement au premier.

### Mécanisme 1 — `STYLE_DESCRIPTIONS` (le style, développé structurellement)

**Signalé par David** en lisant deux articles générés : le format
produit était un mini-article journalistique classique (accroche,
citations, chute), pas une annonce de service. Confirmé dans le
fichier de définition (`thematiques/petites_annonces_services.md`) :
`style_journalistique: utilitaire`, mais rien dans le code
(`prompt_builder.py`, `build_journalistic_brief()`) ne développait ce
mot en consigne structurelle — juste `**Style** : utilitaire`, un mot
brut sans explication de ce que ça doit changer concrètement dans la
forme.

**Rapprochement fait par David avec P21** ("Journaux oraux, orateurs
itinérants", scopé le 12 juillet, jamais codé) : P21 prévoyait déjà un
"registre oral" avec ses propres règles structurelles (pas de chapô,
adresse directe, structure accroche→développement→appel à l'action) —
exactement le niveau de détail qui manquait à `utilitaire`.

**Conception retenue** : nouveau dictionnaire extensible
`STYLE_DESCRIPTIONS` (`prompt_builder.py`), plutôt qu'un correctif
isolé pour cette seule thématique — `utilitaire` développé ("va à
l'essentiel, pas de citation de porte-parole nécessaire, pas de chute,
ton neutre comme une annonce de service public"), structure prête à
accueillir une entrée `"oral"` le jour où P21 sera codé (reste un
chantier à part entière — rien fait aujourd'hui au-delà du socle).

Non-régression : tout style absent du dictionnaire garde le
comportement d'origine (mot brut).

### Mécanisme 2 — `format_fige` (la longueur, indépendant de P21)

**Second problème repéré par David en creusant le premier test** : le
choix manuel de longueur dans le GUI ("analyse", 600-900 mots)
**écrasait** systématiquement le format naturel court de la thématique
— contradiction insoluble pour le LLM (impossible d'écrire une annonce
courte et pratique tout en remplissant 900 mots), qui a fini par
privilégier la longueur au détriment du ton.

**Question de portée tranchée avec David** : le nouveau mécanisme
devait-il s'appliquer à toutes les thématiques, ou seulement
certaines ? Décision, en comparant à la presse réelle : `meteo` et
`petites_annonces_services` sont de vrais **genres** à forme
intrinsèquement courte (pas un choix éditorial au cas par cas).
`actualites_a_la_une`, en revanche, volontairement **laissée
flexible** — "à la une" est une priorité éditoriale, pas un genre ; la
presse réelle laisse sa longueur suivre la substance de l'histoire.

**Conception retenue** : nouveau champ opt-in `format_fige: true` dans
le frontmatter des thématiques concernées (même logique déclarative
que `format_dominant`/`style_journalistique`, plutôt qu'une liste
codée en dur dans `prompt_builder.py` — extensible, pas de code à
toucher pour en figer une nouvelle plus tard).

**Refactoring fait au passage** : la logique de priorité
longueur-override-vs-format-naturel était dupliquée entre
`build_journalistic_brief()` et `build_prompt()` — exactement le genre
de duplication qui avait déjà causé un bug corrigé le 3 août
(`metadata["longueur"]` incohérent avec la vraie consigne). Centralisée
dans une nouvelle fonction `_resoudre_longueur()`, appelée par les deux.

Testé en synthétique sur 4 cas (figée+override ignoré, non
figée+override respecté, non-régression thématique sans le champ,
figée sans override demandé) — tous passent.

### Bug trouvé en testant en conditions réelles — 3e occurrence du même piège aujourd'hui

**Premier test réel : échec silencieux.** `format_fige: true` bien
écrit sur `petites_annonces_services.md`, mais l'article généré restait
long (5330 caractères, `mots_reels: 626` — dans la plage "analyse", pas
"brève"). Cause : `load_thematique()` (`loader.py`) reconstruit le dict
d'une thématique avec une **liste blanche de champs connus** —
exactement le même bug que `load_instance()` trouvé plus tôt dans la
journée pour `garantie_selection`/`priorite_forcee`. `format_fige`
était de fait silencieusement perdu à chaque chargement, malgré une
écriture correcte sur disque. Corrigé (ajouté à la liste).
...
reproduit) → `mots_reels: 221`, dans la bonne plage (200-400), format
transformé en vraie structure d'annonce (titre-annonce, "Mission :",
"Rémunération :", "Équipement fourni :", coordonnées de contact —
plus aucune accroche narrative, citation ou chute). **Test de
non-régression sur `actualites_a_la_une`** (non figée) dans l'intervalle :
comportement inchangé, retry de longueur normal toujours actif.

**Fichiers livrés** : `prompt_builder.py` (cumulé avec tous les autres
correctifs du jour), `loader.py` (`format_fige` ajouté à
`load_thematique()`, cumulé avec les correctifs `garantie_selection`/
`priorite_forcee` du matin).

**`meteo` traité en fin de session, également clos.**
`format_fige: true` était déjà présent sur `thematiques/meteo.md`
(David l'avait ajouté de son côté) — fonctionnel directement une fois
`loader.py` corrigé, aucune action supplémentaire nécessaire sur ce
point. `style_journalistique: informatif` (pas `utilitaire`) restait
en revanche non développé dans `STYLE_DESCRIPTIONS` — nouvelle entrée
ajoutée ("registre factuel et condensé, priorité aux données concrètes
[températures, seuils, dates, zones], pas de développement narratif,
citations courtes seulement si elles apportent une donnée précise,
structure info-clé puis contexte bref puis recommandation pratique —
proche d'un bulletin d'agence"). Testé en synthétique (3 cas :
`informatif` développé, `utilitaire` toujours développé, style non
listé inchangé) puis **en conditions réelles par David : "testé et
validé"**.

---

## ✅ 16. Dashboard GUI affichant "0 articles" — clos (23 août)
**Signalé par David** en observant le dashboard pendant un batch de
test. Cause : `_stats_articles()` et `_stats_thematiques()`
(`routes_dashboard.py`) utilisaient `articles_dir.glob("*.md")` — **non
récursif**. Depuis l'uniformisation du 22 août (point 13,
`generate.py` range désormais dans `articles/{scenario}/` même en
génération unitaire), plus aucun fichier n'existe à la racine
`articles/`, donc ces deux fonctions ne trouvaient plus rien.

**Même classe de bug déjà corrigée le 10 août** pour
`trace_injection.py`/`audit_longueur_articles.py` — jamais répercutée
à `routes_dashboard.py`, qui vit délibérément hors du flux de patches
habituel sur `app.py` (extrait le 4 juillet, voir docstring du module,
justement pour éviter d'être écrasé par les patches fréquents) — ce
qui l'a fait passer sous le radar des deux corrections précédentes.

**Corrigé** : `glob("*.md")` → `glob("**/*.md")` dans les deux
fonctions, plus filtre d'exclusion des `_index.md` (même filtre
qu'`audit_longueur_articles.py`, absent ici jusqu'à ce correctif —
sans lui, les fichiers d'index de série auraient été comptés à tort
comme des articles).

**Testé sur structure synthétique** (`articles/{scenario}/` avec un
`_index.md` mélangé) avant tout déploiement : 2 articles trouvés
(0 avant correctif), `_index.md` bien exclu, répartition par scénario/
thématique/ligne éditoriale correcte.

**Fichier livré** : `routes_dashboard.py`. Redémarrage GUI Flask
nécessaire. Non re-testé sur le vrai dashboard de David après
livraison (à confirmer à la prochaine ouverture du GUI).

---

## ✅ 17. Variété de palette/éclairage/composition + réutilisation des signes distinctifs dans `image_prompt` — codé (23 août), pas encore re-testé en conditions réelles
**Signalé par David** : le champ `image_prompt` des articles fait très
souvent référence à une "lueur bleue" — vérifié : **30/129 articles
mesurés (≈23%)** contiennent du vocabulaire bleu ("lueur bleue",
"teintes azur"...). Confirmé dans le code : la consigne `IMAGE_PROMPT`
(`prompt_builder.py`) ne mentionne aucune couleur/éclairage — tic
stylistique pur du LLM sur les scènes "tech/futuriste" de cet univers,
pas un effet de la consigne.

**Premier correctif** : ligne ajoutée à la consigne demandant de
varier l'éclairage/la palette selon le contexte réel de la scène
(lumière naturelle, dorée, chaude, nocturne, industrielle...) plutôt
que de retomber systématiquement sur du bleu/froid.

**Découverte en creusant plus loin** (question de David : une base de
données d'infrastructures/instances pourrait-elle nourrir
`image_prompt` et inspirer les futurs articles ?) : **ce mécanisme
existe déjà**, sous une autre forme. Le champ `signes_distinctifs`
(présent sur **758/758 instances du vault**, vérifié en direct) est
transmis au LLM pour chaque entité citée depuis le 3 août
(`build_entities_context()`) — mais la consigne `IMAGE_PROMPT` ne
faisait jamais explicitement le lien vers cette section pourtant déjà
présente dans le même prompt. Probable facteur supplémentaire du
réflexe "bleu générique" : à défaut d'instruction de réutilisation, le
LLM invente un visuel depuis zéro plutôt que de piocher dans les
logos/couleurs/symboles déjà établis.

**Second correctif, même fichier** : consigne `IMAGE_PROMPT`
réécrite pour explicitement demander la réutilisation des "Signes
distinctifs" déjà listés dans ENTITÉS CANONIQUES quand l'article porte
sur une entité qui en a, plutôt que d'en inventer de nouveaux.

**Conclusion sur l'idée de base de données d'infrastructures**
(discussion mise en pause puis résolue) : `create_entities_and_instances.py`
a déjà une catégorie `VALID_CATEGORIES` incluant `infrastructure` — le
pipeline entités/instances existant couvre déjà ce besoin
conceptuellement, pas de nouveau système à construire. Le vrai manque
identifié n'était pas l'absence de données, mais l'absence de lien
entre une donnée déjà systématiquement présente
(`signes_distinctifs`) et la consigne `image_prompt` — corrigé
ci-dessus.

**Troisième correctif, même fichier, découvert un peu plus tard dans la
session** (David : "les image_prompt sont tous construits de la même
manière") : mesuré séparément, **83/160 image_prompt (52%) contiennent
le mot "écran"**, avec "salle de" (41) et "hologramme" (29) — motif
dominant confirmé : une salle de contrôle/briefing avec écrans et
hologrammes, quel que soit le sujet réel de l'article. Axe différent de
la palette (composition/lieu de la scène, pas couleur). Même type de
correctif : ligne ajoutée demandant de varier la composition et le lieu
selon le sujet réel (extérieur, rue, terrain, atelier, domicile, gros
plan, foule, paysage...) plutôt que de retomber sur le trope par défaut.

**Aucun test possible en synthétique** (consigne de prompt, pas de
logique testable hors appel LLM réel) — **à vérifier sur un futur
batch normal**, pas de test dédié jugé nécessaire pour un ajustement de
style à faible enjeu (décision de David).

**Fichier livré** : `prompt_builder.py` (cumulé avec tous les autres
correctifs du jour sur ce fichier — un seul remplacement suffit).

---

## ✅ 18. Garde-fou retry pour `signes_distinctifs` manquant — codé et testé (23 août)
**Suite logique du point 17** : David a demandé si les futures
instances auront toujours ce champ rempli. Vérifié dans le code :
**non garanti structurellement** — le schéma JSON envoyé au LLM le
*suggère* sans le marquer requis
(`instance_generation_common.py:751`), `write_instance_file()` a un
repli silencieux sur `""` si absent, et `validate.py` ne vérifie
jamais sa présence/non-vacuité. Le 758/758 actuel est un hasard
statistique favorable, pas une garantie.

**Corrigé** : nouvelle fonction `_retry_with_signes_distinctifs_feedback()`,
même principe que `_retry_with_length_feedback()` (articles, 10 août)
— un seul nouvel appel LLM avec rappel explicite si `signes_distinctifs`
est vide/absent après le premier essai, résultat accepté quoi qu'il
arrive (pas de boucle si le 2e essai échoue aussi). Nouveau champ
`retry_signes_distinctifs: oui/non` écrit dans le frontmatter de
chaque instance — même convention que `retry_longueur` côté articles,
rend le phénomène visible plutôt que silencieux s'il se produit un
jour.

**Testé sur 3 cas synthétiques** (présent du premier coup → pas de
retry ; absent puis récupéré → mis à jour, tracé ; absent aux deux
essais → accepté malgré tout, 2 appels maximum, pas de boucle
infinie) — tous passent.

**Testé en conditions réelles** (entité de test créée via GUI,
`reference`) : `retry_signes_distinctifs: non` avec `signes_distinctifs`
bien rempli — confirme la non-régression du cas normal (le champ
s'écrit proprement dans le frontmatter sans rien casser) mais **ne
prouve pas le déclenchement réel du retry** (taux d'échec naturel trop
faible pour tomber dessus par hasard sur un seul test). Décision de
David : niveau de preuve jugé suffisant (synthétique + non-régression
réelle), pas de test forcé supplémentaire.

**Fichier livré** : `instance_generation_common.py`. Pas de
redémarrage GUI nécessaire (script backend).

**Nettoyage de fin de session** : entité de test supprimée par David
via `undo_custom.py --generalisation yes --execute`.
