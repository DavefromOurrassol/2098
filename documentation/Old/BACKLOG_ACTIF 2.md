...
## 🟡 4. Diagnostic + nettoyage des doublons "pays entier" (`origine_reelle`)
**Ouvert le 13 septembre**, suite à un bug remonté sur l'outil de dessin
de zone complète (S11) : un tracé partiel sur la France colorait le pays
entier sur la carte de base. Diagnostic a mené à deux bugs structurels
distincts dans `zone_repository.py`, tous deux corrigés le jour même :

- `overlay_creer()` — quand un pays n'était pas encore dans
  `origine_reelle` d'une zone, l'entrée créée pour porter le texte
  `portion` était structurellement identique à un vrai rattachement "pays
  entier" (aucun marqueur overlay-only). Corrigé : nouvelle méthode
  `_paires_overlay()` (couples zone/pays réellement backés par un tracé
  dans `geo_overlays/{scenario}.geojson`), utilisée par
  `origine_reelle_index()` pour exclure ces entrées du calcul de couleur
  de base, et par `zones_portant_un_pays()` (enrichie : renvoie
  maintenant `type` pays_entier/overlay + `portion` par zone, plus
  seulement une liste de slugs).
- `overlay_supprimer()` — cliquer "🗑️ supprimer le masque" ne vidait
  (depuis le 12 sept) que le texte `portion`, laissant l'entrée
  `origine_reelle` en place : chaque suppression de masque recréait donc
  un nouveau doublon pays-entier. Corrigé : l'entrée est maintenant
  retirée entièrement quand plus aucun autre masque ne couvre le même
  couple (zone, pays) — garde-fou testé pour le cas d'un pays découpé en
  plusieurs polygones distincts dans la même zone.

Au passage, `supprimer_zone_n1()` a aussi été généralisée à tous les
niveaux (bloquait à tort toute zone != niveau 1, sans raison dans sa
propre logique) — nécessaire pour supprimer une sous-zone
(`Casablanca-Périphérie`, voir plus bas).

**Nouvel outil** : `diagnostiquer_doublons_pays_entier.py` (à placer dans
`gui/`, à côté de `zone_repository.py`/`app.py`). Trois itérations avant
d'être fiable — v1 comparait tous niveaux sans distinguer parent/enfant
(quasi tout le vault remontait en faux positif, sous-zones héritant
normalement de leur racine N1) et relisait tout le vault par pays (lent) ;
v2 corrige les deux mais signalait à tort toute entité absente de
`zones_pays.json` (villes, régions fictives, entités composées type
"Danemark / Groenland") ; v3 ajoute `_resoudre_entite()`, réutilise
`_tokens_entite()` (déjà dans `zone_repository.py` depuis le 14 juillet)
pour reconnaître ces entités composées. Usage : `--scenario X` (rapport,
deux sections pays réels/entités non reconnues), `--nettoyer --pays X
--garder SLUG [--execute]` (ciblé), `--nettoyer-auto [--execute]
[--rapport-portions FICHIER.md]` (lot, cas non ambigus seulement, sauvegarde
les textes portion avant suppression). Nouvelle méthode dédiée
`retirer_doublons_pays_entier()` dans `zone_repository.py` —
**volontairement distincte** de `retirer_pays_des_autres_zones()`
(existante, testée et rejetée pour ce cas : elle retire aussi les
overlays légitimes du pays dans d'autres zones, laisse un masque
orphelin).

Second outil, `retirer_entree_parasite_zones_pays.py` : effet de bord
découvert en nettoyant des entités non-pays — `retirer_doublons_pays_
entier()` met toujours à jour `zones_pays.json`, ce qui ajoute une clé
parasite pour une entité qui n'y a pas sa place (Balkans occidentaux,
Danemark / Groenland en tant que clé composée). Script dédié pour la
retirer proprement (refuse sur un vrai pays, `.bak` automatique).

**Nettoyage réel effectué** : `fortress_world` (12 pays réels + Bruxelles
+ Balkans occidentaux + Casablanca-Périphérie, confirmé propre),
`breakdown` (Sénégal), `eco_communalism` (Algérie, Brésil, Kazakhstan,
Maroc), `new_sustainability` (Internet mondial/GAFAM, Danemark/Groenland
— entrée parasite `zones_pays.json` pas confirmée nettoyée). `policy_reform`
et `reference` **jamais scannés**.

**Décisions actées en cours de route** : `Bruxelles-Forteresse` +
l'ex-ANBA (renommée **ANZES** — "Siège de l'ANZES — Autorité Numérique de
la Zone Euro Sud", slug `anzes_siege_zone_euro_sud`) rattachées toutes
les deux à `Zone Euro Sud` (niveau 2) ; `Balkans occidentaux` gardé sous
`Zone Euro Sud` (via `Zone Tampon Balkano-Caucasienne`) ; `Casablanca-
Périphérie` supprimée entièrement (gardé `Marchés Gris de
Casablanca-Périphérie` sous `Al-Hima`) ; GAFAM gardé sur `Tours de
contrôle orbital Helios`.

**Reste à faire** :
- Scanner `policy_reform` et `reference`.
- Reconfirmer `new_sustainability` propre après le nettoyage Danemark/
  Groenland (dernier scan non relancé depuis).
- Retirer l'entrée parasite `"Danemark / Groenland"` de
  `zones_pays.json["new_sustainability"]`.
- Relire les rapports de textes `portion` retirés
  (`rapport_portions_retirees_13sept.md`,
  `rapport_portions_eco_communalism_13sept.md`) — certains pourraient
  mériter d'être redessinés en overlay plutôt que perdus.
- Fix racine à considérer : `retirer_doublons_pays_entier()` ne devrait
  mettre à jour `zones_pays.json` que si l'entité est un vrai pays
  (`pays_liste`) — contournement en place, pas corrigé à la racine.
- **Intégration GUI, demande explicite de David** : une liste de
  propositions déjà calculées (même patron que le panneau "Zones à
  enrichir" ou la review S11) — jamais de champ où taper un slug/nom à la
  main. Pas encore scopé (onglet dédié vs section du panneau Carte,
  review pays par pays vs bouton "tout nettoyer") — à trancher en début
  de prochaine session.

Voir `HANDOFF_13_SEPTEMBRE.md` pour le détail complet (tests synthétiques
à chaque itération, exemples de vault reproduisant chaque bug avant/après
fix).
