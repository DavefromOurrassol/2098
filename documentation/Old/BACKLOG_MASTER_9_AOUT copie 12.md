...
## 🟡 9bis. `articles_lies` + vocabulaire des tags — codé et lancé, nouveau constat de fond à qualifier
...
**Exécution réelle demandée à David — confirmation du run non-dry-run
à vérifier en début de prochaine session** (l'historique de
conversation ne contient pas explicitement la sortie de la commande
sans `--dry-run`, seulement le dry-run puis directement `--stats` —
`--stats` fonctionne indépendamment sur les `entites_citees`/`tags`
déjà en frontmatter, donc son succès ne prouve pas que `articles_lies`
et `tags_reference.yaml` ont réellement été écrits).
...
dans les articles) de celui des institutions à spectre large. Mérite
peut-être un diagnostic séparé, pas nécessairement la même cause
racine que les institutions.
...
**Décision de David (fin de séance) : documenter pour la prochaine
session plutôt que qualifier/corriger maintenant.** Le constat est
passé du statut "point d'observation isolé" à "pattern structurel
avéré sur la majorité des scénarios" — mérite une vraie session de
discussion (le mécanisme est-il un défaut à corriger, une
caractéristique voulue de certaines entités "tissu conjonctif", ou
les deux selon les cas) plutôt qu'un correctif réflexe en fin de
séance.
