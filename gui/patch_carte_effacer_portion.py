"""
patch_carte_effacer_portion.py — Supprimer un masque efface maintenant aussi
le texte "portion" associé dans origine_reelle (décision David, 12 sept
2026 -- avant, le texte narratif restait orphelin, source de confusion :
un pays affichait toujours sa description de portion alors que le tracé
avait disparu). Corrige le message de confirmation en conséquence (il disait
à tort "origine_reelle reste inchangé").

Nécessite zone_repository.py à jour (overlay_supprimer() efface déjà la
portion -- livré séparément).

Usage, depuis gui/ :
    python3 patch_carte_effacer_portion.py --dry-run
    python3 patch_carte_effacer_portion.py --apply
"""
...
OLD_TEXT = "if (!window.confirm('Retirer ce tracé ? (origine_reelle reste inchangé)')) return;"
NEW_TEXT = "if (!window.confirm('Retirer ce masque ? Le texte de portion associé sera aussi effacé.')) return;"
...
def main():
    ...
    bak = APP_JS.with_suffix(APP_JS.suffix + ".bak14")
