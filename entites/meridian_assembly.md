---
name: Meridian Assembly
type: entity
slug: meridian_assembly
category: institution
description: >
  Une institution post-nationale émergente qui incarne la reconfiguration de la souveraineté par l'abstraction des frontières traditionnelles, en s'appuyant sur des mécanismes de citoyenneté fluide, des réseaux déterritorialisés et une diplomatie opportuniste. Elle représente la montée en puissance d'acteurs hybrides, ni tout à fait étatiques ni purement privés, qui exploitent les failles des systèmes géopolitiques en crise pour s'imposer comme des alternatives légitimes, souvent au détriment des populations locales ou des structures héritées. Son essence réside dans sa capacité à agréger des fragments de pouvoir (reconnaissance internationale, ressources, adhésions) pour former une entité cohérente, mais dont la légitimité reste contestée.
tension_fondamentale: >
  La contradiction entre l'inclusion formelle (citoyenneté par abonnement, sièges diplomatiques) et l'exclusion réelle (détournement de souveraineté, marginalisation des communautés d'origine), où l'institution se nourrit des crises qu'elle prétend résoudre tout en reproduisant des rapports de domination sous une nouvelle forme.
variables_potentielles:
  - gouvernance_institutions
  - geopolitique_conflits
  - demographie_mobilite_humaine
  - technologie_information
scenarios_instances:
  - reference
  - breakdown
  - eco_communalism
  - policy_reform
date_creation: 2026-09-24
custom_source: apocalypse_nerds_2026-09 (Hadjadji & Tesquet, Apocalypse Nerds, 2025)
scenario_ref: reference
role_ref: >
  Assemblée d'États-réseaux nés de communautés en ligne, citoyenneté par abonnement, terres dispersées sur plusieurs continents. Son levier principal : racheter la souveraineté diplomatique des États insulaires submergés des Archipels Flottants du Pacifique Sud pour obtenir sièges et reconnaissance, au grand malaise des communautés déplacées.
etat_ref: ascendant
consignes_scenarios:
  breakdown: 'Entité secondaire dans ce scénario : elle n''existe plus en 2098 qu''à l''état de souvenir des années 2020-2050 (trajectoire historique ou mythifié), évoquée brièvement, jamais acteur de premier plan.'
  eco_communalism: 'Entité secondaire dans ce scénario : elle n''existe plus en 2098 qu''à l''état de souvenir des années 2020-2050 (trajectoire historique ou mythifié), évoquée brièvement, jamais acteur de premier plan.'
  policy_reform: 'Entité secondaire dans ce scénario : elle n''existe plus en 2098 qu''à l''état de souvenir des années 2020-2050 (trajectoire historique ou mythifié), évoquée brièvement, jamais acteur de premier plan.'
---

# Meridian Assembly

## Description archétypale
Une institution post-nationale émergente qui incarne la reconfiguration de la souveraineté par l'abstraction des frontières traditionnelles, en s'appuyant sur des mécanismes de citoyenneté fluide, des réseaux déterritorialisés et une diplomatie opportuniste. Elle représente la montée en puissance d'acteurs hybrides, ni tout à fait étatiques ni purement privés, qui exploitent les failles des systèmes géopolitiques en crise pour s'imposer comme des alternatives légitimes, souvent au détriment des populations locales ou des structures héritées. Son essence réside dans sa capacité à agréger des fragments de pouvoir (reconnaissance internationale, ressources, adhésions) pour former une entité cohérente, mais dont la légitimité reste contestée.

## Tension fondamentale
La contradiction entre l'inclusion formelle (citoyenneté par abonnement, sièges diplomatiques) et l'exclusion réelle (détournement de souveraineté, marginalisation des communautés d'origine), où l'institution se nourrit des crises qu'elle prétend résoudre tout en reproduisant des rapports de domination sous une nouvelle forme.

## Instances par scénario
| Scénario | Instance | État | Rôle |
|---|---|---|---|
| [[breakdown]] | [[meridian_assembly_breakdown]] | | |
| [[fortress_world]] | — | — | — |
| [[new_sustainability]] | — | — | — |
| [[eco_communalism]] | [[meridian_assembly_eco_communalism]] | | |
| [[policy_reform]] | [[meridian_assembly_policy_reform]] | | |
| [[reference]] | [[meridian_assembly_reference]] | | |

