---
name: The Lattice of Chartered Cities
type: instance
slug: the_lattice_reference
entite: the_lattice
scenario: reference
localisation:
  zone: ameriques_multipolaires
  lieu: Amérique centrale
  type_lieu: region

type_dans_scenario: réseau

role_dans_scenario: >
  Fédération de villes à charte d'Amérique centrale apparue dans le vide laissé par la crise constitutionnelle états-unienne de 2055. Chaque cité fonctionne comme une juridiction autonome avec son propre règlement, son conseil de gérance et un droit de sortie garanti, sans élections traditionnelles. Les résidents « votent avec leurs pieds » en changeant de cité, créant un marché de gouvernance où la légitimité découle de la capacité à attirer et retenir des habitants par des règles attractives plutôt que par la coercition ou la représentation électorale.

responsabilites: >
  Gestion décentralisée des infrastructures urbaines, régulation des flux migratoires intra-réseau, arbitrage des conflits entre cités via des mécanismes de médiation algorithmique, et négociation avec les acteurs externes (États résiduels, consortiums énergétiques, plateformes logistiques) pour préserver l'autonomie du Lattice. Le réseau garantit aussi la portabilité des droits sociaux et économiques des résidents lors de leurs déplacements entre cités.

impact_local: 4
impact_systemique_global: 3

variables_influencees:
    - gouvernance_institutions
    - organisation_territoires
    - demographie_mobilite_humaine
    - systeme_economique_redistribution

zone_geographique:
    - régionale
    - urbaine

zone_systemique:
    - gouvernance
    - économie
    - société

alliances:
    - confederation_des_megapoles_autonomes_reference
    - agence_internationale_des_energies_renouvelables_irena_2_reference
    - cooperatives_energetiques_decentralisees_reference
    - mouvement_pour_la_libre_circulation_des_personnes_et_des_donnees_reference

oppositions:
    - coalition_des_gouvernements_contre_les_chartes_de_destabilisation_reference
    - bloc_des_souverainistes_climatiques_reference
    - gouvernements_federaux_residuels_souverainistes_reference

type_relation_dominante: rivalité

annee_debut: 2056
annee_fin: 

trajectoire: mature
est_clandestin: false
generation: reconstruction

injection:
  garantie_selection: false
  type: custom
  annee_injection: 2056
  contexte_injection: >
    Le Lattice incarne une rupture avec les modèles de gouvernance centralisés, accélérant la fragmentation institutionnelle et la reconfiguration des territoires urbains en entités autonomes. Son impact sur la mobilité humaine est direct (droit de sortie garanti, portabilité des droits), tandis que sa logique de marché de gouvernance influence les dynamiques démographiques à l'échelle régionale.
  impact_sur_variables:
  - variable: gouvernance_institutions
    delta_level: 12
    duree: 20
    polarite: 1
  - variable: organisation_territoires
    delta_level: 10
    duree: 15
    polarite: 1
  - variable: demographie_mobilite_humaine
    delta_level: 8
    duree: 10
    polarite: 1
  propagation:
    via_matrice: true

description_journalistique: >
  Depuis la crise de 2055, le Lattice est devenu le laboratoire le plus radical de gouvernance post-nationale. Ses cités-états, comme Nueva Cartago ou Puerto Soberano, rivalisent d'ingéniosité pour attirer résidents et investissements : certaines misent sur des régimes fiscaux ultra-légers, d'autres sur des écosystèmes technologiques fermés ou des droits sociaux étendus. Les « passeports Lattice » sont désormais acceptés dans une vingtaine de pays, et les flux migratoires entre cités se mesurent en temps réel via des tableaux de bord publics. Pourtant, derrière cette façade de fluidité, les tensions persistent : certaines cités deviennent des havres pour élites fuyant les régulations climatiques, tandis que d'autres peinent à retenir leurs habitants face à la concurrence.

signes_distinctifs: >
  Logo : un treillis géométrique stylisé en filigrane sur les documents officiels, symbolisant l'interconnexion des juridictions. Architecture : les cités du Lattice arborent des tours modulaires en acier recyclé, conçues pour être reconfigurées selon les besoins démographiques. Langage : l'espagnol technique mêlé d'anglais et de néologismes juridiques (« charte-droit », « gérance algorithmique ») domine les échanges administratifs.
retry_signes_distinctifs: non

tensions_narratives: >
  1) **Stabilité vs. Fluidité** : Le Lattice repose sur la mobilité des résidents, mais cette fluidité menace la cohérence des politiques publiques (ex. : comment financer des infrastructures durables si les habitants quittent une cité dès qu'une taxe est instaurée ?). 2) **Autonomie vs. Dépendance** : Certaines cités deviennent des satellites de consortiums énergétiques ou logistiques, remettant en cause leur souveraineté. 3) **Inégalités territoriales** : Les cités les plus attractives accumulent richesses et talents, tandis que les autres sombrent dans une spirale de déclin, créant des poches de pauvreté au sein même du réseau. 4) **Légitimité externe** : Les États voisins accusent le Lattice de dumping réglementaire et menacent de fermer leurs frontières aux résidents du réseau.

date_creation: 2026-09-24
---

# The Lattice of Chartered Cities

## Rôle dans [[reference]]
Fédération de villes à charte d'Amérique centrale apparue dans le vide laissé par la crise constitutionnelle états-unienne de 2055. Chaque cité fonctionne comme une juridiction autonome avec son propre règlement, son conseil de gérance et un droit de sortie garanti, sans élections traditionnelles. Les résidents « votent avec leurs pieds » en changeant de cité, créant un marché de gouvernance où la légitimité découle de la capacité à attirer et retenir des habitants par des règles attractives plutôt que par la coercition ou la représentation électorale.

## Responsabilités
Gestion décentralisée des infrastructures urbaines, régulation des flux migratoires intra-réseau, arbitrage des conflits entre cités via des mécanismes de médiation algorithmique, et négociation avec les acteurs externes (États résiduels, consortiums énergétiques, plateformes logistiques) pour préserver l'autonomie du Lattice. Le réseau garantit aussi la portabilité des droits sociaux et économiques des résidents lors de leurs déplacements entre cités.

## Variables influencées
- [[gouvernance_institutions]]
- [[organisation_territoires]]
- [[demographie_mobilite_humaine]]
- [[systeme_economique_redistribution]]

## Relations
**Alliés** : [[confederation_des_megapoles_autonomes_reference]], [[agence_internationale_des_energies_renouvelables_irena_2_reference]], [[cooperatives_energetiques_decentralisees_reference]], [[mouvement_pour_la_libre_circulation_des_personnes_et_des_donnees_reference]]
**Opposants** : [[coalition_des_gouvernements_contre_les_chartes_de_destabilisation_reference]], [[bloc_des_souverainistes_climatiques_reference]], [[gouvernements_federaux_residuels_souverainistes_reference]]

## Description journalistique
Depuis la crise de 2055, le Lattice est devenu le laboratoire le plus radical de gouvernance post-nationale. Ses cités-états, comme Nueva Cartago ou Puerto Soberano, rivalisent d'ingéniosité pour attirer résidents et investissements : certaines misent sur des régimes fiscaux ultra-légers, d'autres sur des écosystèmes technologiques fermés ou des droits sociaux étendus. Les « passeports Lattice » sont désormais acceptés dans une vingtaine de pays, et les flux migratoires entre cités se mesurent en temps réel via des tableaux de bord publics. Pourtant, derrière cette façade de fluidité, les tensions persistent : certaines cités deviennent des havres pour élites fuyant les régulations climatiques, tandis que d'autres peinent à retenir leurs habitants face à la concurrence.

## Tensions narratives
1) **Stabilité vs. Fluidité** : Le Lattice repose sur la mobilité des résidents, mais cette fluidité menace la cohérence des politiques publiques (ex. : comment financer des infrastructures durables si les habitants quittent une cité dès qu'une taxe est instaurée ?). 2) **Autonomie vs. Dépendance** : Certaines cités deviennent des satellites de consortiums énergétiques ou logistiques, remettant en cause leur souveraineté. 3) **Inégalités territoriales** : Les cités les plus attractives accumulent richesses et talents, tandis que les autres sombrent dans une spirale de déclin, créant des poches de pauvreté au sein même du réseau. 4) **Légitimité externe** : Les États voisins accusent le Lattice de dumping réglementaire et menacent de fermer leurs frontières aux résidents du réseau.
