---
name: Alpha47 - Complexe Minier Ergo-Wian
type: instance
slug: alpha47_fortress_world
entite: alpha47
scenario: fortress_world
localisation:
  zone: null
  lieu: null
  type_lieu: null
  note: transnationale_sans_ancrage

type_dans_scenario: infrastructure

role_dans_scenario: >
  Alpha47 est une planète d'extraction minière détenue par Ergo-Wian Sovereign Dominion (EWSD), où des travailleurs recrutés dans le Hors sous contrat de service sont exploités pour l'extraction de ressources critiques. Ce complexe incarne la logique extractiviste des blocs forteresses, où la production prime sur la sécurité humaine et environnementale. Une maladie mystérieuse y frappe les mineurs, générant des visions chez les mourants, mais Ergo-Wian étouffe toute information, illustrant l'opacité systémique et l'irresponsabilité des conglomérats industriels dans les zones sacrifiées.

responsabilites: >
  Extraction de minerais stratégiques (terres rares, métaux critiques) pour les blocs forteresses, gestion des travailleurs sous contrat de service issus des zones marginalisées, maintien du secret sur les conditions sanitaires et les phénomènes inexpliqués, et sécurisation militarisée du site contre les intrusions ou les fuites d'information.

impact_local: 5
impact_systemique_global: 3

variables_influencees:
    - energie_ressources_critiques
    - geopolitique_conflits
    - sante_biotechnologies
    - systemes_productifs_travail

zone_geographique:
    - planétaire

zone_systemique:
    - énergie
    - infrastructure
    - sécurité
    - société

alliances:
    - ergo_wian_sovereign_holdings_fortress_world
    - complexes_militaro_industriels_de_gestion_des_ressources_fortress_world
    - milices_privees_de_protection_des_sites_germinaux_fortress_world
    - vaultcorp_security_fortress_world

oppositions:
    - alliance_sanitaire_des_populations_exclues_fortress_world
    - coalitions_des_deplaces_et_apatrides_fortress_world
    - reseau_des_cartographes_des_zones_grises_fortress_world
    - voix_du_dehors_fortress_world

type_relation_dominante: conflit

annee_debut: 2047
annee_fin: 

trajectoire: mature
est_clandestin: false
generation: forteresse

injection:
  garantie_selection: false
  type: custom
  annee_injection: 2047
  contexte_injection: >
    Alpha47 illustre la logique extractiviste et sécuritaire des blocs forteresses, où la production de ressources critiques prime sur les droits humains et la transparence. Son impact sur energie_ressources_critiques est direct (sécurisation des approvisionnements), tandis que son exploitation des travailleurs aggrave les tensions dans systemes_productifs_travail (précarisation, opacité). Enfin, son rôle dans geopolitique_conflits est croissant, car les ressources qu'elle produit alimentent les rivalités entre blocs et les stratégies de contrôle des zones sacrifiées.
  impact_sur_variables:
  - variable: energie_ressources_critiques
    delta_level: 10
    duree: 20
    polarite: 1
  - variable: systemes_productifs_travail
    delta_level: -8
    duree: 15
    polarite: -1
  - variable: geopolitique_conflits
    delta_level: 5
    duree: 10
    polarite: 1
  propagation:
    via_matrice: true

description_journalistique: >
  Alpha47, surnommée 'La Mine des Ombres' par les travailleurs du Hors, est un complexe minier orbital situé sur une planète stérile du système Eridani. Géré par Ergo-Wian Sovereign Dominion, ce site est un symbole de l'exploitation des populations marginalisées dans les zones sacrifiées des blocs forteresses. Les mineurs, recrutés sous contrat de service dans les camps de réfugiés du Hors, extraient des terres rares et des métaux critiques dans des conditions dignes du XIXe siècle, sous la surveillance de milices privées. Une maladie mystérieuse, baptisée 'Fièvre des Visions', décime les travailleurs, mais Ergo-Wian refuse toute enquête indépendante, préférant étouffer les rumeurs plutôt que de risquer une interruption de la production. Les rares témoignages de survivants évoquent des hallucinations collectives avant la mort, alimentant les théories les plus folles sur une possible contamination extraterrestre ou une arme biologique secrète.

signes_distinctifs: >
  Le complexe est marqué par des tours d'extraction géantes en forme de croix, illuminées en permanence par des projecteurs bleutés, symbolisant la domination d'Ergo-Wian. Les mineurs portent des combinaisons grises standardisées, sans marque distinctive, et sont identifiés par des puces RFID implantées. Les zones résidentielles sont des baraquements préfabriqués, entourés de barbelés et de miradors, tandis que les sites d'extraction sont protégés par des drones armés et des champs de force électromagnétiques. Les symboles de la maladie, des cercles concentriques rouges, sont peints sur les portes des quartiers contaminés.
retry_signes_distinctifs: non

tensions_narratives: >
  Alpha47 est au cœur de plusieurs enjeux majeurs : la lutte pour le contrôle des ressources critiques entre blocs forteresses, la résistance des travailleurs exploités et des réseaux clandestins qui tentent de faire fuiter des informations, et les mystères entourant la 'Fièvre des Visions'. Les rumeurs d'une possible origine non naturelle de la maladie pourraient déclencher une crise sanitaire mondiale si elles étaient confirmées, tandis que les tentatives d'Ergo-Wian pour étouffer l'affaire risquent de provoquer une révolte des mineurs ou une intervention des coalitions des déplacés. Par ailleurs, la découverte éventuelle de phénomènes inexpliqués sur Alpha47 pourrait remettre en question les fondements mêmes de la rationalité scientifique des blocs forteresses.

date_creation: 2026-09-24
---

# Alpha47 - Complexe Minier Ergo-Wian

## Rôle dans [[fortress_world]]
Alpha47 est une planète d'extraction minière détenue par Ergo-Wian Sovereign Dominion (EWSD), où des travailleurs recrutés dans le Hors sous contrat de service sont exploités pour l'extraction de ressources critiques. Ce complexe incarne la logique extractiviste des blocs forteresses, où la production prime sur la sécurité humaine et environnementale. Une maladie mystérieuse y frappe les mineurs, générant des visions chez les mourants, mais Ergo-Wian étouffe toute information, illustrant l'opacité systémique et l'irresponsabilité des conglomérats industriels dans les zones sacrifiées.

## Responsabilités
Extraction de minerais stratégiques (terres rares, métaux critiques) pour les blocs forteresses, gestion des travailleurs sous contrat de service issus des zones marginalisées, maintien du secret sur les conditions sanitaires et les phénomènes inexpliqués, et sécurisation militarisée du site contre les intrusions ou les fuites d'information.

## Variables influencées
- [[energie_ressources_critiques]]
- [[geopolitique_conflits]]
- [[sante_biotechnologies]]
- [[systemes_productifs_travail]]

## Relations
**Alliés** : [[ergo_wian_sovereign_holdings_fortress_world]], [[complexes_militaro_industriels_de_gestion_des_ressources_fortress_world]], [[milices_privees_de_protection_des_sites_germinaux_fortress_world]], [[vaultcorp_security_fortress_world]]
**Opposants** : [[alliance_sanitaire_des_populations_exclues_fortress_world]], [[coalitions_des_deplaces_et_apatrides_fortress_world]], [[reseau_des_cartographes_des_zones_grises_fortress_world]], [[voix_du_dehors_fortress_world]]

## Description journalistique
Alpha47, surnommée 'La Mine des Ombres' par les travailleurs du Hors, est un complexe minier orbital situé sur une planète stérile du système Eridani. Géré par Ergo-Wian Sovereign Dominion, ce site est un symbole de l'exploitation des populations marginalisées dans les zones sacrifiées des blocs forteresses. Les mineurs, recrutés sous contrat de service dans les camps de réfugiés du Hors, extraient des terres rares et des métaux critiques dans des conditions dignes du XIXe siècle, sous la surveillance de milices privées. Une maladie mystérieuse, baptisée 'Fièvre des Visions', décime les travailleurs, mais Ergo-Wian refuse toute enquête indépendante, préférant étouffer les rumeurs plutôt que de risquer une interruption de la production. Les rares témoignages de survivants évoquent des hallucinations collectives avant la mort, alimentant les théories les plus folles sur une possible contamination extraterrestre ou une arme biologique secrète.

## Tensions narratives
Alpha47 est au cœur de plusieurs enjeux majeurs : la lutte pour le contrôle des ressources critiques entre blocs forteresses, la résistance des travailleurs exploités et des réseaux clandestins qui tentent de faire fuiter des informations, et les mystères entourant la 'Fièvre des Visions'. Les rumeurs d'une possible origine non naturelle de la maladie pourraient déclencher une crise sanitaire mondiale si elles étaient confirmées, tandis que les tentatives d'Ergo-Wian pour étouffer l'affaire risquent de provoquer une révolte des mineurs ou une intervention des coalitions des déplacés. Par ailleurs, la découverte éventuelle de phénomènes inexpliqués sur Alpha47 pourrait remettre en question les fondements mêmes de la rationalité scientifique des blocs forteresses.
