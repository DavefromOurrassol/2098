---
name: Ergo-Wian Sovereign Holdings
type: instance
slug: ergo_wian_sovereign_holdings_reference
entite: ergo_wian_sovereign_holdings
scenario: reference
localisation:
  zone: europe_occidentale_reconstructee
  lieu: Rotterdam
  type_lieu: ville

type_dans_scenario: hybride

role_dans_scenario: >
  Ergo-Wian Sovereign Holdings incarne l'aboutissement extrême de la financiarisation de la souveraineté. Dans un monde où les États traditionnels peinent à gérer les crises climatiques, énergétiques et migratoires, cette entité a fusionné gouvernance territoriale et logique corporative pour créer un modèle de 'souveraineté actionnariale'. Elle administre des territoires entiers comme des actifs financiers, où les résidents sont des clients-contractants sans droits politiques, et où les décisions publiques sont prises par un conseil d'actionnaires. Son expansion au-delà de ses frontières initiales, via des contrats de gérance d'infrastructures critiques, en fait un acteur systémique du Pacte des Souverains, redéfinissant les normes de la gouvernance post-démocratique.

responsabilites: >
  Ergo-Wian détient et gère en monopole l'eau, l'énergie, les réseaux de données, la monnaie, l'assurance, l'éducation et la sécurité de ses territoires. Elle rachète ou étouffe toute concurrence, externalise la gestion des populations via des algorithmes de 'productivité sociale', et exporte son modèle en prenant le contrôle d'infrastructures publiques d'autres membres du Pacte des Souverains. Son souverain-gérant, nommé pour dix ans, supervise un système où la résiliation du contrat de résidence équivaut à une expulsion pure et simple.

impact_local: 5
impact_systemique_global: 4

variables_influencees:
    - gouvernance_institutions
    - systeme_economique_redistribution
    - organisation_territoires
    - technologie_information

zone_geographique:
    - nationale
    - régionale
    - continentale

zone_systemique:
    - gouvernance
    - économie
    - IA
    - infrastructure

alliances:
    - pacte_des_souverains_reference
    - consortiums_bancaires_financiarises_reference
    - consortium_des_grandes_plateformes_ia_integrees_reference
    - consortiums_prives_de_gestion_des_donnees_critiques_reference
    - plateformes_d_optimisation_territoriale_par_ia_reference

oppositions:
    - federation_communs_territoriaux_reference
    - banque_des_communs_reference
    - cooperatives_energetiques_decentralisees_reference
    - collectifs_academiques_independants_reference
    - reseau_des_villes_etats_pilotes_en_gouvernance_participative_reference
    - mouvement_pour_la_libre_circulation_des_personnes_et_des_donnees_reference

type_relation_dominante: dépendance

annee_debut: 2047
annee_fin: 

trajectoire: dominant
est_clandestin: false
generation: ère cognitive

injection:
  garantie_selection: false
  type: custom
  annee_injection: 2047
  contexte_injection: >
    Ergo-Wian Sovereign Holdings accélère la fragmentation des institutions démocratiques en normalisant un modèle de gouvernance où le pouvoir politique est capturé par des intérêts économiques. Son impact négatif sur la redistribution est direct : en traitant les populations comme des clients plutôt que des citoyens, elle aggrave les inégalités structurelles. En revanche, son approche centralisée et algorithmique optimise l'organisation des territoires, les rendant plus 'efficients' mais aussi plus vulnérables aux chocs systémiques.
  impact_sur_variables:
  - variable: gouvernance_institutions
    delta_level: -15
    duree: 20
    polarite: -1
  - variable: systeme_economique_redistribution
    delta_level: -12
    duree: 15
    polarite: -1
  - variable: organisation_territoires
    delta_level: 10
    duree: 25
    polarite: 1
  propagation:
    via_matrice: true

description_journalistique: >
  Depuis son siège social flottant au-dessus de l'ancienne ville de Rotterdam, Ergo-Wian Sovereign Holdings règne sur un archipel de territoires où la démocratie a été remplacée par des contrats à durée déterminée. Ses résidents, rebaptisés 'clients-contractants', paient pour l'air conditionné de leurs logements modulaires, pour l'accès à l'eau dessalée, et même pour le droit de respirer un air filtré des particules fines. Les expulsions pour 'non-respect des clauses de productivité' se comptent par milliers chaque année, gérées par des algorithmes qui évaluent en temps réel la 'valeur sociale' de chaque individu. Son modèle, exporté via des partenariats avec des États en faillite, a fait d'Ergo-Wian le premier 'gouvernement-entreprise' coté en Bourse, où les actionnaires votent les lois comme on vote des dividendes.

signes_distinctifs: >
  Son logo, un hexagone doré traversé par une courbe de rendement, est devenu le symbole de la gouvernance algorithmique. Ses territoires sont reconnaissables à leurs gratte-ciels transparents, où les données de consommation des résidents défilent en temps réel sur des écrans géants. Les uniformes de ses 'agents de conformité' – mi-policiers, mi-conseillers clientèle – arborent des badges connectés qui affichent le score de satisfaction des résidents à leur passage.
retry_signes_distinctifs: non

tensions_narratives: >
  Ergo-Wian est au cœur d'une bataille juridique mondiale sur la définition même de la souveraineté : ses détracteurs l'accusent d'être une 'dictature actionnariale', tandis que ses défenseurs y voient l'aboutissement logique d'un monde où l'efficacité économique prime sur les idéaux politiques. Son expansion menace les derniers bastions de gouvernance participative, et son modèle de 'résiliation unilatérale des contrats de résidence' est contesté par des mouvements de résidents qui réclament le droit de vote. Par ailleurs, ses algorithmes de gestion sociale, de plus en plus opaques, sont soupçonnés de discriminations systémiques, alimentant une crise de légitimité qui pourrait faire vaciller son empire.

date_creation: 2026-09-24
---

# Ergo-Wian Sovereign Holdings

## Rôle dans [[reference]]
Ergo-Wian Sovereign Holdings incarne l'aboutissement extrême de la financiarisation de la souveraineté. Dans un monde où les États traditionnels peinent à gérer les crises climatiques, énergétiques et migratoires, cette entité a fusionné gouvernance territoriale et logique corporative pour créer un modèle de 'souveraineté actionnariale'. Elle administre des territoires entiers comme des actifs financiers, où les résidents sont des clients-contractants sans droits politiques, et où les décisions publiques sont prises par un conseil d'actionnaires. Son expansion au-delà de ses frontières initiales, via des contrats de gérance d'infrastructures critiques, en fait un acteur systémique du Pacte des Souverains, redéfinissant les normes de la gouvernance post-démocratique.

## Responsabilités
Ergo-Wian détient et gère en monopole l'eau, l'énergie, les réseaux de données, la monnaie, l'assurance, l'éducation et la sécurité de ses territoires. Elle rachète ou étouffe toute concurrence, externalise la gestion des populations via des algorithmes de 'productivité sociale', et exporte son modèle en prenant le contrôle d'infrastructures publiques d'autres membres du Pacte des Souverains. Son souverain-gérant, nommé pour dix ans, supervise un système où la résiliation du contrat de résidence équivaut à une expulsion pure et simple.

## Variables influencées
- [[gouvernance_institutions]]
- [[systeme_economique_redistribution]]
- [[organisation_territoires]]
- [[technologie_information]]

## Relations
**Alliés** : [[pacte_des_souverains_reference]], [[consortiums_bancaires_financiarises_reference]], [[consortium_des_grandes_plateformes_ia_integrees_reference]], [[consortiums_prives_de_gestion_des_donnees_critiques_reference]], [[plateformes_d_optimisation_territoriale_par_ia_reference]]
**Opposants** : [[federation_communs_territoriaux_reference]], [[banque_des_communs_reference]], [[cooperatives_energetiques_decentralisees_reference]], [[collectifs_academiques_independants_reference]], [[reseau_des_villes_etats_pilotes_en_gouvernance_participative_reference]], [[mouvement_pour_la_libre_circulation_des_personnes_et_des_donnees_reference]]

## Description journalistique
Depuis son siège social flottant au-dessus de l'ancienne ville de Rotterdam, Ergo-Wian Sovereign Holdings règne sur un archipel de territoires où la démocratie a été remplacée par des contrats à durée déterminée. Ses résidents, rebaptisés 'clients-contractants', paient pour l'air conditionné de leurs logements modulaires, pour l'accès à l'eau dessalée, et même pour le droit de respirer un air filtré des particules fines. Les expulsions pour 'non-respect des clauses de productivité' se comptent par milliers chaque année, gérées par des algorithmes qui évaluent en temps réel la 'valeur sociale' de chaque individu. Son modèle, exporté via des partenariats avec des États en faillite, a fait d'Ergo-Wian le premier 'gouvernement-entreprise' coté en Bourse, où les actionnaires votent les lois comme on vote des dividendes.

## Tensions narratives
Ergo-Wian est au cœur d'une bataille juridique mondiale sur la définition même de la souveraineté : ses détracteurs l'accusent d'être une 'dictature actionnariale', tandis que ses défenseurs y voient l'aboutissement logique d'un monde où l'efficacité économique prime sur les idéaux politiques. Son expansion menace les derniers bastions de gouvernance participative, et son modèle de 'résiliation unilatérale des contrats de résidence' est contesté par des mouvements de résidents qui réclament le droit de vote. Par ailleurs, ses algorithmes de gestion sociale, de plus en plus opaques, sont soupçonnés de discriminations systémiques, alimentant une crise de légitimité qui pourrait faire vaciller son empire.
